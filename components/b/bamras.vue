<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="sjoeosb_w"/><path class="bjo10fk9j"/><path class="a4-nsev6j"/><path class="ibarn1b8p"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:bamras" /></template>
<style>
.a4-nsev6j {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M45.5 24c0 6.787-9.626 12.29-21.5 12.29h0C12.126 36.29 2.5 30.787 2.5 24h0c0-6.787 9.626-12.29 21.5-12.29h0c11.874 0 21.5 5.503 21.5 12.29");
}

.bjo10fk9j {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M23.056 18.976s.323.925 0 1.53c-.338.635-.885 1.145-1.649 1.446c-.763.3-1.929.956-1.929 2.518s1.265 1.802 1.93 2.175s1.25.799 1.25 1.665s-.585 1.354-1.25 1.651c-.666.298-1.4.731-1.247 1.535");
}

.ibarn1b8p {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M28.157 28.26v-8.327h1.872c2.01 0 3.64 1.631 3.64 3.643v1.041a3.64 3.64 0 0 1-3.64 3.643zm10.555-8.327v8.327M12.72 24.096a2.081 2.081 0 0 1 0 4.164H9.288v-8.327h3.432a2.081 2.081 0 0 1 0 4.164Zm0 0H9.288");
}

.sjoeosb_w {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M22.106 16.328c.365 0 .59-.522.338-.905l-1.018-1.542h-.006l-1.018 1.542c-.253.383-.027.905.338.905zm-1.412.993h1.458m-1.266 1.135h1.074m-.537.22V32.24M19.79 18.976s-.323.925 0 1.53c.338.635.885 1.145 1.649 1.446c.763.3 1.929.956 1.929 2.518s-1.265 1.802-1.93 2.175s-1.25.799-1.25 1.665s.585 1.354 1.25 1.651c.667.298 1.4.731 1.247 1.535");
}

</style>
