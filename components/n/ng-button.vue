<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":128,"height":128};
const content = `<path class="o205f_btu"/><path class="vlfltdbow"/><path class="g_2zs9bjv"/><path class="l9fbc47ht"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="noto:ng-button" /></template>
<style>
.g_2zs9bjv {
  fill: var(--svg-color--b4e1ed, #b4e1ed);
  d: path("M39.7 12.9c0-2.3-1.6-3-10.8-2.7c-7.7.3-11.5 1.2-13.8 4s-2.9 8.5-3 15.3c0 4.8 0 9.3 2.5 9.3c3.4 0 3.4-7.9 6.2-12.3c5.4-8.7 18.9-10.6 18.9-13.6");
  opacity: var(--svg-opacity--0-5, 0.5);
}

.l9fbc47ht {
  fill: var(--svg-color--fafafa, #fafafa);
  d: path("M17.67 41.22h10.21l18.72 31.2h.52l-.52-8.97V41.22h8.71v46.54h-9.23L26.32 54.81h-.52l.52 8.97v23.99h-8.65zm56.78 44.43c-3.75-2.1-6.73-5-8.94-8.71S62.2 69.09 62.2 64.5s1.1-8.74 3.31-12.45s5.19-6.61 8.94-8.71s7.85-3.15 12.32-3.15c3.64 0 6.97.64 9.98 1.92q4.515 1.92 7.77 5.43l-6.04 5.92c-1.56-1.65-3.28-2.88-5.17-3.71q-2.82-1.23-6.6-1.23c-2.86 0-5.48.66-7.87 1.98c-2.38 1.32-4.28 3.21-5.69 5.65c-1.41 2.45-2.11 5.23-2.11 8.35s.71 5.9 2.14 8.35s3.35 4.33 5.75 5.66c2.41 1.32 5.04 1.98 7.9 1.98c4.33 0 7.89-1.28 10.66-3.83c.87-.78 1.62-1.78 2.27-2.99s1.13-2.53 1.43-3.96H86.57v-7.47h22.81c.3 1.21.46 2.58.46 4.09c0 3.08-.47 5.94-1.4 8.58q-1.395 3.96-4.26 6.96c-2.08 2.25-4.59 3.98-7.54 5.17s-6.24 1.79-9.88 1.79c-4.46-.03-8.56-1.07-12.31-3.18");
}

.o205f_btu {
  fill: var(--svg-color--427687, #427687);
  d: path("M116 4H12c-4.42 0-8 3.58-8 8v104c0 4.42 3.58 8 8 8h104c4.42 0 8-3.58 8-8V12c0-4.42-3.58-8-8-8");
}

.vlfltdbow {
  fill: var(--svg-color--8cafbf, #8cafbf);
  d: path("M109.7 4H11.5A7.555 7.555 0 0 0 4 11.5v97.9c-.01 4.14 3.34 7.49 7.48 7.5h98.12c4.14.01 7.49-3.34 7.5-7.48V11.5c.09-4.05-3.13-7.41-7.18-7.5z");
}

</style>
