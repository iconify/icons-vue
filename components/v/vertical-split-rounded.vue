<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="kc2ejpbaa"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:vertical-split-rounded" /></template>
<style>
.kc2ejpbaa {
  fill: currentColor;
  d: path("M4.5 14.346q-.213 0-.356-.144T4 13.846t.144-.356t.356-.144h6q.213 0 .356.144q.144.144.144.357t-.144.356t-.356.143zM4.5 18q-.213 0-.356-.144T4 17.499t.144-.356T4.5 17h6q.213 0 .356.144t.144.357t-.144.356T10.5 18zm0-7.346q-.213 0-.356-.144Q4 10.366 4 10.153t.144-.356t.356-.143h6q.213 0 .356.144t.144.356t-.144.356t-.356.144zM4.5 7q-.213 0-.356-.144T4 6.499t.144-.356T4.5 6h6q.213 0 .356.144t.144.357t-.144.356T10.5 7zm11.116 11q-.667 0-1.141-.475T14 16.386v-8.77q0-.666.475-1.14T15.615 6h2.77q.666 0 1.14.475T20 7.615v8.77q0 .666-.475 1.14t-1.14.475z");
}

</style>
