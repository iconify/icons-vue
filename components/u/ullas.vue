<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="lrt-uhbhe"/><path class="k9n08rvmy"/><path class="mh99mbcrx"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:ullas" /></template>
<style>
.k9n08rvmy {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M28.733 22.162c1.13-.56 2.39-.333 2.805.504c.423.846-.162 1.976-1.293 2.546c-1.13.56-2.39.333-2.805-.504c-.423-.846.162-1.976 1.293-2.546m-.147-4.098c.968-.643 2.155-.545 2.651.211s.114 1.879-.845 2.521c-.968.634-2.155.537-2.651-.22c-.496-.756-.114-1.878.845-2.512m-9.668-2.367c-.643-.292-1.293-.577-2.033.537c-.74 1.106-1.586 3.627-2.033 5.855s-.496 4.172-.22 5.896c.277 1.724.886 3.236 1.626 4.22c.732.992 1.602 1.464 2.375 1.675c.78.22 1.472.18 2.18-.049s1.422-.65 1.992-1.04c.569-.39.984-.749 1.398-1.115");
  opacity: var(--svg-opacity--0-992, 0.992);
}

.lrt-uhbhe {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M23.382 22.666c.61 1.545 1.212 3.082 2.041 5.286c.83 2.196 1.87 5.058 2.033 5.855s-.553-.463-1.04-1.065c-.489-.602-.757-.537-1.066-.52c-.31.007-.667-.025-.96.024c-.3.057-.544.203-.683.496s-.17.732-.195.927c-.024.203-.049.162.3.602c.359.447 1.09 1.366 1.7 2.049c.619.675 1.106 1.106 1.806 1.691c.707.594 1.61 1.35 2.301 1.7c.683.357 1.155.325 1.96.048c.805-.268 1.952-.772 2.7-1.39c.748-.61 1.106-1.334 1.48-2.285c.374-.96.764-2.147.634-3.383c-.138-1.228-.797-2.505-1.35-3.903c-.561-1.407-1.016-2.928-1.488-3.513s-.943-.22-1.22 0s-.341.3-.406.536c-.065.228-.13.61-.293.643c-.163.04-.423-.277-.716-.415c-.292-.138-.618-.098-.813.138s-.268.667-.423.756c-.162.082-.406-.178-.618-.341a1.05 1.05 0 0 0-.602-.244c-.22-.024-.487-.008-.56.35c-.074.366.048 1.073-.204.528s-.894-2.35-1.269-3.52c-.382-1.164-.504-1.684-.723-2.066a1.64 1.64 0 0 0-.87-.748c-.334-.139-.692-.155-.92-.066c-.227.09-.325.285-.39.61c-.073.326-.105.773-.146 1.22");
  opacity: var(--svg-opacity--0-992, 0.992);
}

.mh99mbcrx {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M29.489 13.038c1.968-.528 2.44 2.66-.366 4.611l-.04-11.54H18.917v23.974h7.318M9.566 18.365L7.533 34.344l8.538-2.131m22.363-14.385l2.033 16.516l-4.066.528m0 3.196l2.44-1.065l3.659 2.13l-3.643 2.757l-2.61-.862l-30.406.586l-.341-5.14l19.517-.536");
  opacity: var(--svg-opacity--0-992, 0.992);
}

</style>
