<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="eu89gqbbf"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:ambulance-sharp" /></template>
<style>
.eu89gqbbf {
  fill: currentColor;
  d: path("M11 4V0h2v4zM6.575 5.425l-3-3L5 1l3 3zM6 18h4v-2H6zm8 0h4v-2h-4zm3.275-6.75L20.75 7.8L19.7 6.75l-2.425 2.375l-.975-.975l-1.05 1.075zm-2.812 1.288Q13 11.075 13 9t1.463-3.537T18 4t3.538 1.463T23 9t-1.463 3.538T18 14t-3.537-1.463M3 23v-9l2.45-7H9V5h3.275q-.6.85-.937 1.863T11 9H6.85l-1.475 4h6.9q1 1.425 2.5 2.213T18 16q.8 0 1.538-.162T21 15.325V23h-3v-2H6v2z");
}

</style>
