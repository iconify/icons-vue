<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":16,"height":16};
const content = `<path class="j622l9bkh"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:mail-off-16-regular" /></template>
<style>
.j622l9bkh {
  fill: currentColor;
  d: path("m12.274 12.981l1.872 1.873a.5.5 0 0 0 .708-.708l-13-13a.5.5 0 1 0-.708.708L2.74 3.447C2.288 3.814 2 4.373 2 5v6a2 2 0 0 0 2 2h8q.14 0 .274-.019M11.293 12H4a1 1 0 0 1-1-1V6.876L7.763 9.44a.5.5 0 0 0 .474 0l.323-.173zM7.404 8.111L3 5.74V5a1 1 0 0 1 .455-.838zM13 5.74L9.66 7.538l.738.739L13 6.876v4.003l.86.86c.09-.23.14-.478.14-.739V5a2 2 0 0 0-2-2H5.121l1 1H12a1 1 0 0 1 1 1z");
}

</style>
