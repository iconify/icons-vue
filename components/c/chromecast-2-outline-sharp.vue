<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="tbpl-ac0q"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:chromecast-2-outline-sharp" /></template>
<style>
.tbpl-ac0q {
  fill: currentColor;
  d: path("M9 21q-2.925 0-4.962-2.037T2 14q0-2.65 1.713-4.612T8 7.074V5.7q0-1.55 1.075-2.625T11.7 2q.85 0 1.613.375T14.6 3.4l5.15 6.5l.575-.475L23.1 12.9l-2.75 2.175L17.6 11.6l.575-.475l-5.15-6.5q-.25-.3-.587-.462T11.7 4q-.725 0-1.212.488T10 5.7v1.375q2.575.35 4.288 2.313T16 14q0 2.925-2.037 4.963T9 21m0-2q2.075 0 3.538-1.463T14 14t-1.463-3.537T9 9t-3.537 1.463T4 14t1.463 3.538T9 19");
}

</style>
