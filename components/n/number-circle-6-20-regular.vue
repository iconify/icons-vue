<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="oq105fnwm"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:number-circle-6-20-regular" /></template>
<style>
.oq105fnwm {
  fill: currentColor;
  d: path("M10 3a7 7 0 1 0 0 14a7 7 0 0 0 0-14m-8 7a8 8 0 1 1 16 0a8 8 0 0 1-16 0m8-.5a2 2 0 1 0 0 4c1.208 0 2-.88 2-2s-.792-2-2-2m0 5c-.862 0-1.759-.29-2.434-1.04C6.895 12.714 6.5 11.579 6.5 10c0-1.171.264-2.28.839-3.113C7.927 6.035 8.827 5.5 10 5.5c1.623 0 2.417 1.091 2.682 1.563a.5.5 0 0 1-.871.49C11.621 7.218 11.094 6.5 10 6.5c-.827 0-1.427.36-1.839.955c-.402.584-.636 1.414-.66 2.383A3 3 0 0 1 10 8.5c1.792 0 3 1.36 3 3s-1.208 3-3 3");
}

</style>
