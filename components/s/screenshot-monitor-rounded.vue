<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="hko14mivf"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:screenshot-monitor-rounded" /></template>
<style>
.hko14mivf {
  fill: currentColor;
  d: path("M17.5 14.5h-1.75q-.325 0-.537.213T15 15.25t.213.538t.537.212H18q.425 0 .713-.288T19 15v-2.25q0-.325-.213-.537T18.25 12t-.537.213t-.213.537zm-11-7h1.75q.325 0 .538-.213T9 6.75t-.213-.537T8.25 6H6q-.425 0-.712.288T5 7v2.25q0 .325.213.538T5.75 10t.538-.213t.212-.537zM4 19q-.825 0-1.412-.587T2 17V5q0-.825.588-1.412T4 3h16q.825 0 1.413.588T22 5v12q0 .825-.587 1.413T20 19h-4v1q0 .425-.288.713T15 21H9q-.425 0-.712-.288T8 20v-1z");
}

</style>
