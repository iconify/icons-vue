<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="mzpq-cg6v"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:air-purifier-sharp" /></template>
<style>
.mzpq-cg6v {
  fill: currentColor;
  d: path("M3 21V7q0-1.65 1.175-2.825T7 3h7q1.65 0 2.825 1.175T18 7v1h-2v9h2v4zm10.325-6.175Q14.5 13.65 14.5 12t-1.175-2.825T10.5 8T7.675 9.175T6.5 12t1.175 2.825T10.5 16t2.825-1.175m-4.237-1.412Q8.5 12.825 8.5 12t.588-1.412T10.5 10t1.413.588T12.5 12t-.587 1.413T10.5 14t-1.412-.587M17 12v-2h.9q.225 0 .4-.112t.3-.338q.375-.725 1.037-1.138T21.1 8h.9v2h-.9q-.225 0-.4.113t-.3.337q-.375.725-1.037 1.138T17.9 12zm0 4v-2h.9q.225 0 .4-.112t.3-.338q.375-.725 1.037-1.138T21.1 12h.9v2h-.9q-.225 0-.4.113t-.3.337q-.375.725-1.037 1.138T17.9 16z");
}

</style>
