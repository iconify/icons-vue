<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="zhjzfxbnp"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:do-not-touch-outline" /></template>
<style>
.zhjzfxbnp {
  fill: currentColor;
  d: path("m20 17.208l-1-1V5.5q0-.213.144-.356T19.501 5t.356.144T20 5.5zm-11-11l-1-1v-.15q0-.213.144-.356t.357-.144t.356.144t.143.356zm3.673 3.673l-1-1V3.058q0-.213.144-.356t.357-.144t.356.144t.143.356zm3.673 1.66h-1V4.058q0-.213.144-.356q.144-.144.357-.144t.356.144t.143.356zm1.945 8.17L9 10.422v5.043l-4.411-3.081l5.078 7.456q.202.315.533.488q.33.173.716.173H16.5q.539 0 1.007-.216q.468-.217.783-.572M10.917 21.5q-.624 0-1.164-.28t-.908-.808l-6.125-8.98l.216-.209q.32-.321.76-.348t.809.233L8 13.537V9.42L1.887 3.308l.713-.714l19.223 19.223l-.713.714l-2.12-2.12q-.48.491-1.11.79t-1.38.299zm3.411-9.96");
}

</style>
