<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="dw9nog6et"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:bromite" /></template>
<style>
.dw9nog6et {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M39.17 12.88a1.41 1.41 0 0 0 1.77-.35l1.18-1.85a1.41 1.41 0 0 0-.33-1.77l-9.7-6.24a1.41 1.41 0 0 0-1.77.33l-1.18 1.86a1.43 1.43 0 0 0 .33 1.77l.85.6l-1.69 2.69a17.7 17.7 0 0 0-4.81-.67a18.13 18.13 0 1 0 12.83 5.31l1.51-2.29Zm-.84 14.49a14.6 14.6 0 0 1-.6 4.15a12.2 12.2 0 0 1-5.22 1.17a12.92 12.92 0 0 1-8.26-3.21a22 22 0 0 0-3.91-2.69a21.9 21.9 0 0 0-10.42-2.87a14.41 14.41 0 0 1 28.41 3.45");
}

</style>
