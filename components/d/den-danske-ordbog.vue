<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="i9clfwm2k"/><path class="jldiul8md"/><path class="bk3lcrb-m"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:den-danske-ordbog" /></template>
<style>
.bk3lcrb-m {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M36.9 23.534c.318 1.695-.9 5.002-2.173 5.874c1.218-.457 1.577.014 1.577.788s-1.66 3.487-2.02 5.286c1.066-1.813 1.66-2.297 1.66-2.297s.056 3.81-.415 6.203c.443-1.716 1.19-5.152 1.938-5.774c.553.65.985 3.89.996 3.348c.221-1.895.982-3.542.982-3.542s.623.664.969 1.743c-.028-3.251-.526-8.481-3.515-11.629m-2.394.551c-.375-.13-.658-.55-1.484-.55c-.68 0-1.086.524-1.086.524m-20.404 9.585c-1.68-.9-3.064-2.077-3.064-3.132s.667-1.366 1.401-1.366s1.663 1.067 1.663 1.067s.929-1.067 1.663-1.067s1.401.31 1.401 1.366s-1.384 2.231-3.064 3.132m13.955 4.919s-4.677-6.584-5.742-7.48s-1.454-.482-2.381-1.281c-.415-.358-.43-1.09-.288-1.822c.176-.91.103-1.692.556-2.23c.551-.657 1.54-.145 2.038-.166s1.438-.57 2.024-.078s1.144 2.42 1.144 2.42s1.188 1.179 2.229 2.414c1.04 1.234 2.248 2.652 3.945 3.83");
}

.i9clfwm2k {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M38.5 5.5h-29a4 4 0 0 0-4 4v29a4 4 0 0 0 4 4h29a4 4 0 0 0 4-4v-29a4 4 0 0 0-4-4");
}

.jldiul8md {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M11.323 20.821v-11h2.475a4.81 4.81 0 0 1 4.812 4.813v1.375a4.81 4.81 0 0 1-4.812 4.812zm0-11H9.775m1.548 11H9.775M18.876 42.5s-.903-.551-2.296-.551s-1.423.566-2.647.55c-.547-.006-1.03-.557-1.392-1.208c-.451-.811-1.01-1.362-1.01-2.066c0-.856 1.087-1.101 1.454-1.438s.734-1.362 1.5-1.362s2.432 1.117 2.432 1.117s1.668.138 3.26.413s4.054.949 8.675.398c-.123-1.469-1.01-5.63 3.764-8.69c0 0-5.019.703-6.518-3.49c0 0 .657-1.078 2.585-2.73c.678-.581 1.29-1.623 2.318-2.043c0 0-2.983-1.584-2.119-4.988c0 0 1.286.826 3.244 2.203c.306-1.102 1.714-2.938 1.714-2.938s1.407 1.775 1.836 3.275c0 0 1.989-.98 4.131-2.204c0 0-.98 3.458-2.999 4.805c1.699.168 2.594 1.637 3.099 3.06S42.5 28.24 42.5 28.24");
}

</style>
