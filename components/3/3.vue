<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":128,"height":128};
const content = `<linearGradient id="svgIDa" x1="63.991" x2="63.991" y1="101.5" y2="26.12" gradientTransform="matrix(1 0 0 -1 0 128)" gradientUnits="userSpaceOnUse"><stop offset="0" class="vlzxxcccm"/><stop offset=".515" class="j1rdzyw_l"/></linearGradient><path fill="url(#svgIDa)" class="b1m08qbqx"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="noto:3" /></template>
<style>
.b1m08qbqx {
  d: path("M55.22 60.36a2 2 0 0 1 2-2h5.52c3.79-.03 6.8-1.02 9.02-2.95s3.33-4.72 3.33-8.38c0-3.52-.92-6.26-2.77-8.2c-1.85-1.95-4.67-2.92-8.46-2.92c-3.32 0-6.05.96-8.18 2.87c-1.71 1.53-2.73 3.44-3.07 5.71c-.15 1.01-.97 1.78-1.99 1.78h-8.37c-1.18 0-2.13-1.01-2-2.18c.32-2.95 1.29-5.67 2.91-8.18c2.01-3.12 4.83-5.55 8.43-7.29C55.19 26.87 59.23 26 63.7 26c7.38 0 13.18 1.85 17.4 5.56s6.32 8.86 6.32 15.47c0 3.32-1.06 6.44-3.18 9.37c-1.37 1.9-3 3.49-4.89 4.78c-1.2.82-1.12 2.66.16 3.36c2.4 1.3 4.33 2.98 5.79 5.04c2.12 2.98 3.18 6.54 3.18 10.67c0 6.64-2.28 11.92-6.83 15.85c-4.54 3.94-10.53 5.9-17.95 5.9c-7.11 0-12.94-1.9-17.48-5.69c-4.01-3.35-6.24-7.67-6.71-12.97a2.01 2.01 0 0 1 2-2.17h8.38c1.02 0 1.85.78 1.99 1.79c.34 2.44 1.39 4.49 3.16 6.14c2.18 2.03 5.12 3.05 8.81 3.05c3.83 0 6.84-1.02 9.04-3.05s3.3-4.98 3.3-8.84c0-3.89-1.15-6.89-3.45-8.99s-5.72-3.15-10.26-3.15h-5.26a2 2 0 0 1-2-2v-5.76z");
}

.j1rdzyw_l {
  stop-color: var(--svg-color--504f4f, #504F4F);
}

.vlzxxcccm {
  stop-color: var(--svg-color--757575, #757575);
}

</style>
