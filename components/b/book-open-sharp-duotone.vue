<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="cuyn6tgcc"><path clip-rule="evenodd" class="jwe_uqybk"/><path class="fdeg_urhj"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="keyline-icons:book-open-sharp-duotone" /></template>
<style>
.cuyn6tgcc {
  fill: currentColor;
}

.fdeg_urhj {
  d: path("M22 3C22.5523 3 23 3.4477 23 4V17C23 17.5523 22.5523 18 22 18H15C13.8954 18 13 18.8954 13 20C13 20.5523 12.5523 21 12 21V16H13V4.001C13.8557 3.3591 14.9054 3 16 3H22Z");
}

.jwe_uqybk {
  fill-opacity: var(--svg-fill-opacity--0-4, 0.4);
  fill-rule: evenodd;
  d: path("M2 3L8 3C9.5738 3 11.0557 3.741 12 5C12.9443 3.741 14.4262 3 16 3L22 3C22.5523 3 23 3.4477 23 4L23 17C23 17.5523 22.5523 18 22 18L15 18C13.8954 18 13 18.8954 13 20C13 20.5523 12.5523 21 12 21C11.4477 21 11 20.5523 11 20C11 18.8954 10.1046 18 9 18L2 18C1.4477 18 1 17.5523 1 17L1 4C1 3.4477 1.4477 3 2 3ZM11 4L11 16L13 16L13 4C12.621 4.2843 12.2843 4.621 12 5C11.7157 4.621 11.379 4.2843 11 4Z");
}

</style>
