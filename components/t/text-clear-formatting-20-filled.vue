<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="yiq41xzxj"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:text-clear-formatting-20-filled" /></template>
<style>
.yiq41xzxj {
  fill: currentColor;
  d: path("M6 2a.5.5 0 0 1 .466.318l3.5 8.956a.5.5 0 0 1-.931.364l-1.03-2.635H4.008l-1.042 2.68a.5.5 0 1 1-.932-.363l3.5-9.001A.5.5 0 0 1 6 2m.002 1.877L4.397 8.003h3.217zM11.5 2a.5.5 0 0 1 .5.5v3.523h.004q.338-.609.926-.953q.595-.345 1.332-.345q1.332 0 2.15.994q.819.994.818 2.643q0 .213-.013.414l-.044-.044a2.5 2.5 0 0 0-1.154-.656q-.06-.997-.533-1.614q-.54-.703-1.474-.703q-.899 0-1.474.724q-.436.551-.538 1.366v1.03q.08.645.367 1.123L11 11.37V2.5a.5.5 0 0 1 .5-.5m4.966 7.439l2.095 2.095a1.5 1.5 0 0 1 0 2.122l-3.6 3.599l-4.215-4.217l3.598-3.598a1.5 1.5 0 0 1 2.122-.001m-2.211 8.523l-4.216-4.216l-.599.598a1.5 1.5 0 0 0 0 2.122l2.094 2.095c.308.308.717.454 1.121.438V19h4.092a.5.5 0 1 0 0-1h-2.53z");
}

</style>
