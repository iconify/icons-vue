<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="xono3kb4i"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:article-shortcut-outline-rounded" /></template>
<style>
.xono3kb4i {
  fill: currentColor;
  d: path("M19 21h-8q-.425 0-.712-.288T10 20t.288-.712T11 19h8V5H5v3q0 .425-.288.713T4 9t-.712-.288T3 8V5q0-.825.588-1.412T5 3h14q.825 0 1.413.588T21 5v14q0 .825-.587 1.413T19 21M6.625 22q-1.975 0-3.363-1.388T1.875 17.25q0-1.425.738-2.55T4.55 13H3q-.425 0-.712-.288T2 12t.288-.712T3 11h4q.425 0 .713.288T8 12v4q0 .425-.288.713T7 17t-.712-.288T6 16v-1.425q-.925.2-1.525.95t-.6 1.725q0 1.15.812 1.95t1.938.8q.425 0 .713.288t.287.712t-.288.713t-.712.287M11 17h2q.425 0 .713-.288T14 16t-.288-.712T13 15h-2q-.425 0-.712.288T10 16t.288.713T11 17m0-4h5q.425 0 .713-.288T17 12t-.288-.712T16 11h-5q-.425 0-.712.288T10 12t.288.713T11 13M8 9h8q.425 0 .713-.288T17 8t-.288-.712T16 7H8q-.425 0-.712.288T7 8t.288.713T8 9");
}

</style>
