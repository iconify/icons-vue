<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="wbydzxblo"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:globe-location-pin-outline-rounded" /></template>
<style>
.wbydzxblo {
  fill: currentColor;
  d: path("M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2q3.525 0 6.188 2.113T21.675 9.5q.125.4-.088.75t-.612.475t-.75-.1T19.75 10q-.475-1.825-1.713-3.25T15 4.6V5q0 .825-.587 1.413T13 7h-2v2q0 .425-.288.713T10 10H8v2h6q.375 0 .575.338t0 .687q-.3.575-.437 1.2T14 15.5q0 1.325.6 2.488t1.325 2.212q.2.3.125.65t-.425.475q-.85.35-1.763.513T12 22m-1-2.05V18q-.825 0-1.412-.587T9 16v-1l-4.8-4.8q-.075.45-.137.9T4 12q0 3.025 1.988 5.3T11 19.95m8.2 1.95q-.125-.1-.175-.25q-.275-.875-.775-1.625t-1.075-1.475q-.525-.65-.85-1.425T16 15.5q0-1.45 1.025-2.475T19.5 12t2.475 1.025T23 15.5q0 .85-.337 1.613t-.838 1.437q-.575.725-1.075 1.475t-.775 1.625q-.05.15-.175.25t-.3.1t-.3-.1m.3-2.725q.25-.425.55-.788t.575-.737q.35-.475.613-1.012T21.5 15.5q0-.825-.587-1.412T19.5 13.5t-1.412.588T17.5 15.5q0 .6.263 1.138t.612 1.012l.588.738q.287.363.537.787m-.888-2.787q-.362-.363-.362-.888t.363-.888t.887-.362t.888.363t.362.887t-.363.888t-.887.362t-.888-.363");
}

</style>
