<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":32,"height":32};
const content = `<path class="g48zoqb8t"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:mail-alert-32-filled" /></template>
<style>
.g48zoqb8t {
  fill: currentColor;
  d: path("M19.729 5A5 5 0 0 1 29.5 6.5v2.657l2.308 2.596A.75.75 0 0 1 31.299 13h-13.6a.75.75 0 0 1-.508-1.247L19.5 9.156V6.5c0-.523.08-1.026.229-1.5m4.77 11.5c-.55 0-1.053-.188-1.444-.5q-.24-.192-.417-.439a2.07 2.07 0 0 1-.383-1.061h4.49c-.075 1.079-.987 1.94-2.123 1.997q-.06.003-.122.003M18 6.5q.001-.776.175-1.5H6.5a4.5 4.5 0 0 0-4.496 4.303l1.476.82L16 16.864l4.39-2.364h-2.738l-.048-.003c-1.846-.118-2.793-2.325-1.534-3.74L18 8.585zm12 16v-8h-1.751l-.008.103C28.106 16.563 26.423 18 24.5 18a3.79 3.79 0 0 1-3.19-1.723l-4.836 2.604a1 1 0 0 1-.948 0L2.514 11.874L2 11.588V22.5A4.5 4.5 0 0 0 6.5 27h19a4.5 4.5 0 0 0 4.5-4.5");
}

</style>
