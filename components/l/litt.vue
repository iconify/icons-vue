<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="ft5dv1b6b"><path fill="url(#SVGwhPS2bcc)" class="j5bdbbbfj"/><defs><linearGradient id="SVGwhPS2bcc" x1="8.07" x2="19.731" y1="5.083" y2="12.913" gradientUnits="userSpaceOnUse"><stop class="ne3i2nbzg"/><stop offset=".51" class="mup0boi9a"/><stop offset="1" class="iiru55bfp"/></linearGradient></defs></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="token-branded:litt" /></template>
<style>
.ft5dv1b6b {
  fill: none;
}

.iiru55bfp {
  stop-color: var(--svg-color--92e5eb, #92e5eb);
}

.j5bdbbbfj {
  d: path("M10.74 7.157h2.561c.416-.046 1.19-.48 1.002-1.874c-.005-.037-.06-.055-.093-.023a2.9 2.9 0 0 1-1.024.692c.046-.069.138-.304.138-.706c0-.396-.355-.876-.54-1.066a1.27 1.27 0 0 1-.263-1.116a.046.046 0 0 0-.07-.055a3.14 3.14 0 0 0-1.67 2.662a1.5 1.5 0 0 1-.42-.715c-.008-.046-.073-.06-.096-.019c-.401.646-.784 1.837.475 2.22m-.756.715v.67c0 .045.06.138.276.198c.217.055.346.341.356.475v2.907c0 .032-.028.088-.074.143a4.5 4.5 0 0 0-3.156 4.254C7.386 18.993 9.453 21 12 21s4.614-2.007 4.614-4.48a4.5 4.5 0 0 0-3.151-4.255c-.046-.056-.079-.11-.079-.148V9.215c.01-.134.148-.415.36-.475c.217-.06.277-.157.277-.199v-.669a.25.25 0 0 0-.25-.254h-3.538a.25.25 0 0 0-.25.254m-1.8 8.098a2.04 2.04 0 0 1 2.076-1.624c.974 0 1.357.461 1.74.937c.397.489.803.983 1.86.983c.35-.006.696-.084 1.015-.231c.355-.208.853-.139.89.383v.101c0 2.045-1.707 3.701-3.806 3.701a3.756 3.756 0 0 1-3.77-4.245zm1.97 1.338a.924.924 0 1 0 0-1.847a.924.924 0 0 0 0 1.847m2.769.462a.461.461 0 1 0 0-.923a.461.461 0 0 0 0 .923");
}

.mup0boi9a {
  stop-color: var(--svg-color--47b6ca, #47b6ca);
}

.ne3i2nbzg {
  stop-color: var(--svg-color--33aac2, #33aac2);
}

</style>
