<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":128,"height":128};
const content = `<defs><path id="SVGEuqCNcrv" class="s0b74bbyt"/></defs><circle class="axudp51jy"/><circle class="znafracxo"/><use href="#SVGEuqCNcrv" class="pfsdhbbki"/><path class="iy7cj461x"/><use href="#SVGEuqCNcrv" class="e0ty9gbgt"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="noto:virgo" /></template>
<style>
.axudp51jy {
  cx: 64px;
  cy: 64px;
  r: 60px;
  fill: var(--svg-color--689f38, #689f38);
}

.e0ty9gbgt {
  opacity: var(--svg-opacity--0-65, 0.65);
}

.iy7cj461x {
  fill: var(--svg-color--fafafa, #fafafa);
  d: path("M91.4 96.7c-3.5-1-7-3.9-8.5-8.1c-.1-.4-.1-.8.2-1.1c11.4-11.1 20-25.4 19.8-35.8c-.1-6.2-3.2-10.8-8.7-12.8c-4.8-1.8-9-1.2-12.4.3c-.6.3-1.2-.1-1.4-.7c-1.6-7.2-8-12.5-15.6-12.5c-4.4 0-8.4 1.8-11.3 4.7c-.4.4-1 .4-1.4 0c-3-3-7.2-4.9-11.8-4.7c-8.5.3-15.5 7.9-15.5 16.4v43.4c0 2.1 1.5 3.9 3.6 4.2c2.4.2 4.4-1.6 4.4-4V42.3c0-4.3 3.2-8 7.5-8.3c4.7-.3 8.5 3.4 8.5 8v43.8c0 2.1 1.5 3.9 3.6 4.2c2.4.2 4.4-1.6 4.4-4V42.3c0-4.3 3.2-8 7.5-8.3c4.7-.3 8.6 3.4 8.6 8c0 0-.1 38.8.7 42.8c.1.4-.1.7-.3 1c-4.3 3.6-9 6.5-13.7 8.4c-1.9.8-3.1 2.9-2.5 4.9c.5 1.8 2.1 2.8 3.8 2.8q.75 0 1.5-.3c4.4-1.8 8.8-4.4 13.1-7.6c.5-.4 1.2-.2 1.5.3c1.9 4.2 6 9 12.5 10.4c.3.1.5.1.8.1c2.2 0 4.7-2.3 4.5-4.8c-.2-1.6-1.7-2.9-3.4-3.3m4.5-43.5c-1.6 9-8.9 19.3-14.9 24.9c-.5-5.5-1.5-24.4.2-28.4c4.6-9.8 16.1-4.2 14.7 3.5");
}

.pfsdhbbki {
  opacity: var(--svg-opacity--0-3, 0.3);
}

.s0b74bbyt {
  fill: var(--svg-color--bfe094, #bfe094);
  d: path("M24 29.7c4.5-7.1 14.1-13 24.1-14.8c2.5-.4 5-.6 7.1.2c1.6.6 2.9 2.1 2 3.8c-.7 1.4-2.6 2-4.1 2.5c-9.3 3-17.9 6.4-23.1 14.3c-2 3-4.9 14.4-8.6 12.3c-3.9-2.3-3.1-9.5 2.6-18.3");
}

.znafracxo {
  cx: 60.1px;
  cy: 63.1px;
  r: 56.1px;
  fill: var(--svg-color--9ccc65, #9ccc65);
}

</style>
