<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="jawti_bhv"/><path class="po4rxdejj"/><circle class="vi_nxfq0r"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:firefox-beta" /></template>
<style>
.jawti_bhv {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M33.92 43.788A21.6 21.6 0 0 1 24 46.283c-5.364 0-21.5-3.667-21.5-22.301a17.75 17.75 0 0 1 8.17-15.014a14.3 14.3 0 0 0 .717 5.458h.071a17.8 17.8 0 0 1 5.124-4.049a13 13 0 0 0-.262 2.508a11 11 0 0 0 .322 2.64a4 4 0 0 1 .49.43a11.1 11.1 0 0 0 2.628 2.27c2.484 1.433 4.24 2.257 4.24 2.257s-1.194 2.389-2.64 2.389c-5.9 0-7.083 3.834-7.083 3.834s1.194 7.931 9.556 7.931c3.081 0 9.173-1.696 9.173-8.827a8.84 8.84 0 0 0-4.623-7.883a7.45 7.45 0 0 1 5.614 1.636a11.42 11.42 0 0 0-9.997-5.9h-.705a15.81 15.81 0 0 1 7.43-11.945a23.9 23.9 0 0 0 3.738 5.71c2.604 2.95 5.972 6.51 7.358 10.177a24.6 24.6 0 0 0-2.066-6.366S45.5 14.606 45.5 24.914a21.6 21.6 0 0 1-1.968 8.722");
}

.po4rxdejj {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M39.372 38.5a2 2 0 0 1 0 4h-3.3v-8h3.3a2 2 0 0 1 0 4m0 0h-3.3");
}

.vi_nxfq0r {
  cx: 38.5px;
  cy: 38.5px;
  r: 7px;
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
}

</style>
