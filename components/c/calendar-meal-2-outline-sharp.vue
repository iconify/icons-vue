<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="hfs_u5iap"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:calendar-meal-2-outline-sharp" /></template>
<style>
.hfs_u5iap {
  fill: currentColor;
  d: path("M10.35 15q-.875 0-1.387-.55T8.25 13h7.5q-.2.9-.712 1.45T13.65 15zm-1.275-4q.175-.45.55-.725T10.5 10h1.325q.675-.675.938-.837T13.5 9q.625 0 1.063.438T15 10.5v.5zm1.275 6h3.3q2.275 0 3.288-1.713T18 11h-1v-.5q0-1.45-1.025-2.475T13.5 7q-.725 0-1.362.263T11 8h-.5q-1.3 0-2.275.863T7.05 11H6q.05 2.575 1.063 4.288T10.35 17M3 21V3h3V1h2v2h8V1h2v2h3v18zm16-2V5H5v14zM5 19V5z");
}

</style>
