<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="dg-ou2b-h"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:phone-disabled" /></template>
<style>
.dg-ou2b-h {
  fill: currentColor;
  d: path("m19.723 21.239l-5.32-5.32q-2.243 1.973-4.625 3.027T5.069 20q-.484 0-.777-.3T4 18.95v-2.434q0-.427.254-.728q.254-.302.662-.396l2.103-.43q.352-.07.659.026t.564.354l2.139 2.177q.796-.428 1.715-1.077q.92-.648 1.589-1.242L2.86 4.377l.708-.708L20.43 20.531zm-3.244-7.491l-.713-.713q.407-.502.862-1.197q.455-.694.782-1.246l-2.194-2.046q-.258-.238-.343-.572q-.084-.333-.015-.674l.496-2.384q.094-.408.395-.662T16.477 4h2.473q.45 0 .75.31t.3.76q0 2.23-.97 4.509t-2.551 4.169");
}

</style>
