<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":64,"height":64};
const content = `<path class="ftcofqlpz"/><path class="owyu5pzyw"/><path class="axtpvpb1d"/><path class="ghmrb2bhj"/><path class="c7a0lnbmw"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="emojione:flag-for-flag-cape-verde" /></template>
<style>
.axtpvpb1d {
  fill: var(--svg-color--ed4c5c, #ed4c5c);
  d: path("M4.5 44h55c.8-1.9 1.5-3.9 1.9-6H2.6c.4 2.1 1.1 4.1 1.9 6");
}

.c7a0lnbmw {
  fill: var(--svg-color--ffce31, #ffce31);
  d: path("m25 26.1l1.2.9l-.4-1.5l1.2-1h-1.5L25 23l-.5 1.5H23l1.2 1l-.4 1.5zm0 30l1.2.9l-.4-1.5l1.2-1h-1.5L25 53l-.5 1.5H23l1.2 1l-.4 1.5zm11-20l1.2.9l-.4-1.5l1.2-1h-1.5L36 33l-.5 1.5H34l1.2 1l-.4 1.5zm-18-7l1.2.9l-.4-1.5l1.2-1h-1.5L18 26l-.5 1.5H16l1.2 1l-.4 1.5zm14 0l1.2.9l-.4-1.5l1.2-1h-1.5L32 26l-.5 1.5H30l1.2 1l-.4 1.5zm-19 7l1.2.9l-.4-1.5l1.2-1h-1.5L13 33l-.5 1.5H11l1.2 1l-.4 1.5zm23 12l1.2.9l-.4-1.5l1.2-1h-1.5L36 45l-.5 1.5H34l1.2 1l-.4 1.5zm-23 0l1.2.9l-.4-1.5l1.2-1h-1.5L13 45l-.5 1.5H11l1.2 1l-.4 1.5zm18 6l1.2.9l-.4-1.5l1.2-1h-1.5L31 51l-.5 1.5H29l1.2 1l-.4 1.5zm-13 0l1.2.9l-.4-1.5l1.2-1h-1.5L18 51l-.5 1.5H16l1.2 1l-.4 1.5z");
}

.ftcofqlpz {
  fill: var(--svg-color--2a5f9e, #2a5f9e);
  d: path("M32 62c9.8 0 18.5-4.7 24-12H8c5.5 7.3 14.2 12 24 12m0-60C15.4 2 2 15.4 2 32h60C62 15.4 48.6 2 32 2");
}

.ghmrb2bhj {
  fill: var(--svg-color--fff, #fff);
  d: path("M4.5 44c.9 2.1 2.1 4.2 3.5 6h48c1.4-1.8 2.6-3.9 3.5-6z");
}

.owyu5pzyw {
  fill: var(--svg-color--fff, #fff);
  d: path("M61.4 38q.6-2.85.6-6H2q0 3.15.6 6z");
}

</style>
