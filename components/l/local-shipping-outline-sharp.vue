<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="fhm-b2bea"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:local-shipping-outline-sharp" /></template>
<style>
.fhm-b2bea {
  fill: currentColor;
  d: path("M3.875 19.125Q3 18.25 3 17H1V4h16v4h3l3 4v5h-2q0 1.25-.875 2.125T18 20t-2.125-.875T15 17H9q0 1.25-.875 2.125T6 20t-2.125-.875m2.838-1.412Q7 17.425 7 17t-.288-.712T6 16t-.712.288T5 17t.288.713T6 18t.713-.288M3 15h.8q.425-.45.975-.725T6 14t1.225.275T8.2 15H15V6H3zm15.713 2.713Q19 17.425 19 17t-.288-.712T18 16t-.712.288T17 17t.288.713T18 18t.713-.288M17 13h4.25L19 10h-2zm-8-2.5");
}

</style>
