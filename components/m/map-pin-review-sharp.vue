<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="k3hm_k6_k"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:map-pin-review-sharp" /></template>
<style>
.k3hm_k6_k {
  fill: currentColor;
  d: path("M2 22v-2h3.45q-.7-.65-1.475-1.475T2.55 16.75t-1.1-1.95T1 12.75q0-2.45 1.6-4.1T6.6 7t4 1.65t1.6 4.1q0 1.05-.45 2.05t-1.1 1.95t-1.425 1.775T7.75 20H22v2zm4.6-8q.625 0 1.063-.437T8.1 12.5t-.437-1.062T6.6 11t-1.062.438T5.1 12.5t.438 1.063T6.6 14m8.4 4l-1.45-2.325q.3-.725.475-1.463t.175-1.462q0-.575-.075-1.15t-.225-1.1l1.2-.725L17.125 11L16.6 8.7l1.8-1.575l-2.375-.2L15.1 4.75l-.925 2.175l-2.275.2q-.8-.775-1.775-1.288T8 5.125V2h14v13h-5.125z");
}

</style>
