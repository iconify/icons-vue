<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="fwf5i8eqp"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:meal-dinner-outline-sharp" /></template>
<style>
.fwf5i8eqp {
  fill: currentColor;
  d: path("M8.05 14q-2.5 0-4.275-1.75T2 8t1.775-4.25T8.075 2h.4q-.55.625-.85 1.4t-.3 1.6q0 1.875 1.313 3.188T11.824 9.5q.575 0 1.125-.137T14 8.95q-.35 2.2-2.025 3.625T8.05 14m-.025-2q.6 0 1.188-.162t1.087-.513Q8.125 10.8 6.75 9.063T5.325 5.1q-.65.575-.987 1.325T4 8q0 1.65 1.175 2.825T8.025 12M18 11h2V8h-2zm1 4q.425 0 .713-.288T20 14v-1h-2v1q0 .425.288.713T19 15m-7.55 3h-6.9l.5 2h5.9zM1 22v-2h2l-1-4h12l-1 4h5v-3.175q-.9-.325-1.45-1.1T16 14V6h6v8q0 .95-.55 1.725t-1.45 1.1V20h3v2zm7-2");
}

</style>
