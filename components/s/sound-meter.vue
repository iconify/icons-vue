<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="a55ctnbdn"/><circle class="vf5u6xb7z"/><path class="texsneb9t"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:sound-meter" /></template>
<style>
.a55ctnbdn {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M7.042 25.576H4.5m3.907 2.749l-3.61.637m3.61-6.136l-3.61-.636m5.491-4.53l-3.175-1.834m6.71-2.379l-2.357-2.809m7.119.06l-1.254-3.446M24 9.743V6.077m5.416 4.621l1.254-3.445m3.508 6.195l2.357-2.81m1.177 7.022l3.176-1.833m-1.295 7l3.61-.636m-3.61 6.135l3.61.637M7.042 25.576H4.5m3.564-5.8l-2.388-.87m5.334-4.23l-1.948-1.634m6.458-2.152l-1.27-2.201m6.805.187l-.441-2.503m6.331 2.503l.441-2.503m5.094 4.517l1.27-2.201m3.24 5.987l1.948-1.634m.998 6.734l2.388-.87m-1.366 6.67H43.5");
}

.texsneb9t {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("m20.908 28.171l2.084-1.749m2.117-1.686l8.67-7.365M21.663 38.52a2.01 2.01 0 0 0-2.003-2.002a2.01 2.01 0 0 0-2.002 2.002v1.301c0 1.102.901 2.003 2.002 2.003a2.01 2.01 0 0 0 2.003-2.003m0 2.103v-8.009m6.677 3.904c1.1 0 2.002.9 2.002 2.002s-.901 2.002-2.002 2.002h-3.304v-8.008h3.304c1.1 0 2.002.9 2.002 2.002s-.901 2.002-2.002 2.002m0 0h-3.304");
}

.vf5u6xb7z {
  cx: 24px;
  cy: 25.576px;
  r: 1.316px;
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
}

</style>
