<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="c7yo_pbuv"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:dishwasher-20-filled" /></template>
<style>
.c7yo_pbuv {
  fill: currentColor;
  d: path("M6 3a3 3 0 0 0-3 3v2h14V6a3 3 0 0 0-3-3zm8 14H6.915A1.5 1.5 0 0 0 6 16.085v-1.422A3.5 3.5 0 0 0 8 11.5v-2a1.5 1.5 0 0 0-.086-.5H17v5a3 3 0 0 1-3 3M6.75 6.25a.75.75 0 1 1 0-1.5a.75.75 0 0 1 0 1.5M10 5.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 1-.5-.5M2.5 9a.5.5 0 0 0-.5.5v2a2.5 2.5 0 0 0 2 2.45V17h-.5a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1H5v-3.05a2.5 2.5 0 0 0 2-2.45v-2a.5.5 0 0 0-.5-.5z");
}

</style>
