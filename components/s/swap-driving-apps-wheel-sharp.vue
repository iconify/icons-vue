<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="ppo-_x6ij"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:swap-driving-apps-wheel-sharp" /></template>
<style>
.ppo-_x6ij {
  fill: currentColor;
  d: path("M8.125 21.213q-1.825-.788-3.187-2.15t-2.15-3.188T2 11.988t.788-3.875t2.15-3.175t3.187-2.15T12.013 2t3.875.788t3.175 2.15t2.15 3.175t.787 3.875t-.787 3.887t-2.15 3.188t-3.175 2.15t-3.875.787t-3.888-.787m-.413-10.5Q8 10.425 8 10t-.288-.712T7 9t-.712.288T6 10t.288.713T7 11t.713-.288m3-3Q11 7.426 11 7t-.288-.712T10 6t-.712.288T9 7t.288.713T10 8t.713-.288m7 3Q18 10.426 18 10t-.288-.712T17 9t-.712.288T16 10t.288.713T17 11t.713-.288m-4.3 2.7Q14 12.826 14 12q0-.325-.1-.638t-.3-.562l1.75-4.35l-1.875-.825L11.75 10q-.75.125-1.25.688T10 12q0 .825.588 1.413T12 14t1.413-.587M17.6 17.7q-1.25-.825-2.675-1.263T12 16t-2.925.438T6.4 17.7L8.15 19q.875-.5 1.85-.75t2-.25t2 .25t1.85.75z");
}

</style>
