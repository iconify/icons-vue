<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="xrz9hbcvj"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="ic:outline-not-listed-location" /></template>
<style>
.xrz9hbcvj {
  fill: currentColor;
  d: path("M12.01 16a1 1 0 0 1-.71-.29a.95.95 0 0 1-.29-.71c-.01-.55.43-.99.98-1h.02c.28 0 .51.1.71.29c.18.19.28.43.28.7s-.1.51-.29.71s-.43.3-.7.3m-.88-3.66c0-.45.1-.84.29-1.16c.19-.33.53-.7 1-1.12c.28-.25.48-.47.61-.66s.19-.4.19-.64c0-.29-.11-.53-.32-.74c-.21-.2-.5-.3-.85-.3c-.37 0-.74.1-.96.3c-.21.2-.4.45-.4.98H9c0-1.01.46-1.73.97-2.21C10.53 6.28 11.25 6 12 6c.59 0 1.11.12 1.57.35s.79.55 1.05.96s.38.86.38 1.35s-.1.9-.31 1.25s-.48.71-.89 1.09c-.32.3-.53.56-.65.77s-.18.49-.18.81V13h-1.85v-.66zM18 10.2C18 6.57 15.35 4 12 4s-6 2.57-6 6.2c0 2.34 1.95 5.44 6 9.14c4.05-3.7 6-6.8 6-9.14M12 2c4.2 0 8 3.22 8 8.2c0 3.32-2.67 7.25-8 11.8c-5.33-4.55-8-8.48-8-11.8C4 5.22 7.8 2 12 2");
}

</style>
