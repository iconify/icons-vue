<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":36,"height":36};
const content = `<path class="dgh7jhswh"/><path class="nl7xv-b2o"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="twemoji:aries" /></template>
<style>
.dgh7jhswh {
  fill: var(--svg-color--9266cc, #9266cc);
  d: path("M36 32a4 4 0 0 1-4 4H4a4 4 0 0 1-4-4V4a4 4 0 0 1 4-4h28a4 4 0 0 1 4 4z");
}

.nl7xv-b2o {
  fill: var(--svg-color--fff, #fff);
  d: path("M30.069 7.711c-.863-1.397-2.322-2.188-4.033-2.188c-3.595 0-5.682 3.404-6.456 10.358C19.438 17.166 18.542 24.447 18 28c0 0-1.596-10.834-1.739-12.119c-.773-6.954-2.86-10.358-6.455-10.358c-1.711 0-3.17.79-4.033 2.188c-.918 1.485-1.026 3.478-.282 4.966a1.504 1.504 0 0 0 2.69-1.346c-.24-.481-.527-2.956 1.66-2.956c2.196 0 3.197 5.019 3.688 7.791S15.999 29.994 16 30c.229.938.857 2 2 2s1.833-1.021 1.984-2c.006-.041 1.652-10.023 2.328-13.834c.492-2.772 1.492-7.791 3.688-7.791c2.187 0 1.9 2.475 1.66 2.956a1.503 1.503 0 0 0 .673 2.014a1.503 1.503 0 0 0 2.017-.668c.745-1.488.637-3.481-.281-4.966");
}

</style>
