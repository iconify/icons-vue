<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="w693_8bnj"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:bookmark-flag-rounded" /></template>
<style>
.w693_8bnj {
  fill: currentColor;
  d: path("M10.692 13.25v-2.558h3.385q.242 0 .354-.217t-.031-.429L13.616 9l.784-1.046q.142-.212.03-.429t-.353-.217h-3.461q-.349 0-.578.23t-.23.578v5.134q0 .19.126.316t.316.126t.316-.126t.126-.316M12 16.923l-3.738 1.608q-.808.348-1.535-.134Q6 17.916 6 17.052V5.616q0-.691.463-1.153T7.616 4h8.769q.69 0 1.153.463T18 5.616v11.436q0 .864-.727 1.345q-.727.482-1.535.134z");
}

</style>
