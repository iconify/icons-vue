<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":256,"height":256};
const content = `<g class="cuyn6tgcc"><path class="gxbmosbgh"/><path class="fkdty2bdy"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="ph:user-sound-duotone" /></template>
<style>
.cuyn6tgcc {
  fill: currentColor;
}

.fkdty2bdy {
  d: path("M144 165.68a68 68 0 1 0-71.9 0c-20.65 6.76-39.23 19.39-54.17 37.17a8 8 0 0 0 12.25 10.3C50.25 189.19 77.91 176 108 176s57.75 13.19 77.88 37.15a8 8 0 1 0 12.25-10.3c-14.95-17.78-33.53-30.41-54.13-37.17M56 108a52 52 0 1 1 52 52a52.06 52.06 0 0 1-52-52m151.36-42.4a108.36 108.36 0 0 1 0 84.8a8 8 0 0 1-7.36 4.86a8 8 0 0 1-7.36-11.15a92.26 92.26 0 0 0 0-72.22a8 8 0 0 1 14.72-6.29M248 108a139 139 0 0 1-11.29 55.15a8 8 0 0 1-14.7-6.3a124.43 124.43 0 0 0 0-97.7a8 8 0 1 1 14.7-6.3A139 139 0 0 1 248 108");
}

.gxbmosbgh {
  d: path("M168 108a60 60 0 1 1-60-60a60 60 0 0 1 60 60");
  opacity: var(--svg-opacity--0-2, 0.2);
}

</style>
