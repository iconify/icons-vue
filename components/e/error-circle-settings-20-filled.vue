<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="wgh-gybar"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:error-circle-settings-20-filled" /></template>
<style>
.wgh-gybar {
  fill: currentColor;
  d: path("M18 10a8 8 0 1 0-7.411 7.979A5.5 5.5 0 0 1 10 15.5a5.5 5.5 0 0 1 .216-1.532a.748.748 0 0 1-.966-.718a.75.75 0 0 1 1.408-.36a5.5 5.5 0 0 1 7.32-2.301Q18 10.297 18 10M9.508 6.41a.5.5 0 0 1 .984 0l.008.09V11l-.008.09a.5.5 0 0 1-.984 0L9.5 11V6.5zm2.127 8.51a2 2 0 0 0 1.43-2.478l-.155-.557q.382-.293.821-.497l.338.358a2 2 0 0 0 2.91.001l.324-.343q.448.211.835.518l-.126.422a2 2 0 0 0 1.456 2.519l.349.082a4.7 4.7 0 0 1 .01 1.017l-.46.118a2 2 0 0 0-1.431 2.478l.156.556q-.383.295-.822.498l-.337-.358a2 2 0 0 0-2.91-.002l-.325.345a4.3 4.3 0 0 1-.835-.518l.127-.423a2 2 0 0 0-1.456-2.519l-.35-.083a4.7 4.7 0 0 1-.01-1.016zm4.865.58a1 1 0 1 0-2 0a1 1 0 0 0 2 0");
}

</style>
