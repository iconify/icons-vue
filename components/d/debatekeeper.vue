<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="t-ph8_nmp"/><path class="i482hnxos"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:debatekeeper" /></template>
<style>
.i482hnxos {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M19.358 7.58a2.707 2.707 0 0 0-2.251 4.212a9.89 9.89 0 0 0-7.64 9.625v9.304l-2.548 2.065a1.82 1.82 0 0 0 1.146 3.234h11.293m-8.36-26.599A15.82 15.82 0 0 0 4.5 19.886M19.358 7.58v32.84m0-14.292a7.831 7.831 0 0 0 0-15.663m0 18.666c7.34 0 12.423 1.628 15.247 5.43a2.57 2.57 0 0 1 .475 1.533v2.646a1.68 1.68 0 0 1-1.68 1.68H19.358m14.271-16.963a9.9 9.9 0 0 0 0-10.32m6.288 15.36a16.31 16.31 0 0 0 0-20.4");
}

.t-ph8_nmp {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M17.242 36.02a2.67 2.67 0 0 0-.592 1.691a2.71 2.71 0 0 0 2.708 2.709");
}

</style>
