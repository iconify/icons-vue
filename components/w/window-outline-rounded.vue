<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="nz9hf18is"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:window-outline-rounded" /></template>
<style>
.nz9hf18is {
  fill: currentColor;
  d: path("M5.616 20q-.672 0-1.144-.472T4 18.385V5.615q0-.67.472-1.143Q4.944 4 5.616 4h12.769q.67 0 1.143.472q.472.472.472 1.144v12.769q0 .67-.472 1.143q-.472.472-1.143.472zm6.884-7.5V19h5.885q.269 0 .442-.173t.173-.442V12.5zm0-1H19V5.616q0-.27-.173-.443T18.385 5H12.5zm-1 0V5H5.616q-.27 0-.443.173T5 5.616V11.5zm0 1H5v5.885q0 .269.173.442t.443.173H11.5z");
}

</style>
