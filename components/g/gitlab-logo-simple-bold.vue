<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":256,"height":256};
const content = `<path class="wbuc6sb8m"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="ph:gitlab-logo-simple-bold" /></template>
<style>
.wbuc6sb8m {
  fill: currentColor;
  d: path("M234 116.09L214.13 40a15.94 15.94 0 0 0-30.42-1.48L167 84H89L72.29 38.49A15.94 15.94 0 0 0 41.87 40L22 116.09a61.19 61.19 0 0 0 23.57 65.23l73.27 51.77a15.93 15.93 0 0 0 18.36 0l73.27-51.77A61.19 61.19 0 0 0 234 116.09m-37.4 45.63L128 210.19l-68.6-48.47a37.08 37.08 0 0 1-14.19-39.56l13.4-51.3l10.76 29.28A12 12 0 0 0 80.64 108h94.72a12 12 0 0 0 11.27-7.86l10.76-29.28l13.4 51.3a37.08 37.08 0 0 1-14.19 39.56");
}

</style>
