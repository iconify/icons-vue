<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="dheut1bwb"/><path class="vrz5mraiy"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:emoji-satellite" /></template>
<style>
.dheut1bwb {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M17.172 17.244c2.104-2.104 2.278-5.451.504-7.76a.585.585 0 0 0-.875-.059l-7.32 7.32a.585.585 0 0 0 .042.863a5.67 5.67 0 0 0 7.65-.364m-4.954 25.09L5.667 35.78a.58.58 0 0 1 0-.819l8.745-8.745a.58.58 0 0 1 .819 0l6.552 6.552a.58.58 0 0 1 0 .82l-8.745 8.744a.58.58 0 0 1-.82 0M32.77 21.783l-6.553-6.552a.58.58 0 0 1 0-.82l8.745-8.744a.58.58 0 0 1 .82 0l6.552 6.552a.58.58 0 0 1 0 .819l-8.746 8.745a.58.58 0 0 1-.819 0m.41 4.025l-7.371 7.37a.58.58 0 0 1-.82 0l-8.19-8.19a.58.58 0 0 1 0-.818l7.372-7.372a.58.58 0 0 1 .819 0l8.19 8.19a.58.58 0 0 1 0 .82");
}

.vrz5mraiy {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("m24.17 16.799l-7.372 7.37a.58.58 0 0 1-.819 0l-2.457-2.456a.58.58 0 0 1 0-.82l7.372-7.37a.58.58 0 0 1 .819 0l2.457 2.456a.58.58 0 0 1 0 .82m9.828 12.285l-4.914 4.914a.58.58 0 0 1-.82 0l-1.228-1.229a.58.58 0 0 1 0-.819l4.914-4.914a.58.58 0 0 1 .82 0l1.228 1.229a.58.58 0 0 1 0 .819m-14.134-1.031l-1.395 1.402M29.46 18.457l-1.407 1.407m-17.944-8.467v4.735m6.136-6.151l-4.84.265m-2.317 0a1.158 1.158 0 1 0 2.317 0a1.158 1.158 0 0 0-2.317 0");
}

</style>
