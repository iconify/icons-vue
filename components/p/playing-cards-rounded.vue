<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="bedp99bxh"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:playing-cards-rounded" /></template>
<style>
.bedp99bxh {
  fill: currentColor;
  d: path("m15.067 13.471l.564-2.03q.061-.243-.022-.495q-.084-.252-.301-.394l-1.721-1.183q-.162-.13-.361-.053t-.255.29l-.563 2.03q-.062.243.022.495t.3.394l1.722 1.183q.162.13.36.053q.2-.078.255-.29M4.616 19.287l-.422-.189q-.794-.344-1.069-1.141q-.275-.798.075-1.553l1.416-3.035zM9 20.769q-.825 0-1.412-.59Q7 19.589 7 18.764v-4.725l2.214 6.08q.074.195.144.347q.069.153.194.303zm4.362-.971q-.64.237-1.243-.036t-.838-.914L7.22 7.7q-.236-.64.046-1.24t.923-.835l6.41-2.346q.64-.237 1.24.046t.835.923l4.066 11.148q.236.64-.037 1.24q-.273.599-.914.835z");
}

</style>
