<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="nrj6p8qat"><path class="psvpw2rsi"/><path class="imzag9cmd"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="keyline-icons:caret-right-two-tone" /></template>
<style>
.imzag9cmd {
  d: path("M17.7413 12.456L6.7841 18.9273C6.43652 19.1326 6 18.8787 6 18.4713L6 5.52868C6 5.12126 6.43651 4.86742 6.7841 5.0727L17.7413 11.544C18.0862 11.7477 18.0862 12.2523 17.7413 12.456Z");
}

.nrj6p8qat {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  stroke-width: var(--svg-stroke-width--2px, 2px);
}

.psvpw2rsi {
  fill: currentColor;
  fill-opacity: var(--svg-fill-opacity--0-4, 0.4);
  d: path("M6.8105 4.0249C6.9397 4.0507 7.0688 4.0951 7.1953 4.1587L7.293 4.2124L18.25 10.6831C18.3749 10.7568 18.4844 10.8436 18.5781 10.9409C19.2348 11.6229 19.1253 12.8009 18.25 13.3178L7.293 19.7885C6.9056 20.0173 6.4861 20.0526 6.1133 19.9458C5.989 19.9102 5.8699 19.8586 5.7588 19.7934C5.315 19.5329 5.0003 19.0514 5 18.4721L5 5.5288C5.0002 4.8267 5.4619 4.2675 6.0557 4.0717C6.1743 4.0326 6.2983 4.0079 6.4248 3.9995C6.5516 3.991 6.6812 3.999 6.8105 4.0249Z");
  stroke: none;
}

</style>
