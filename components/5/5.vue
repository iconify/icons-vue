<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":128,"height":128};
const content = `<linearGradient id="svgIDa" x1="64.005" x2="64.005" y1="101.5" y2="25.76" gradientTransform="matrix(1 0 0 -1 0 128)" gradientUnits="userSpaceOnUse"><stop offset="0" class="vlzxxcccm"/><stop offset=".515" class="j1rdzyw_l"/></linearGradient><path fill="url(#svgIDa)" class="rk7z1_b-f"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="noto:5" /></template>
<style>
.j1rdzyw_l {
  stop-color: var(--svg-color--504f4f, #504F4F);
}

.rk7z1_b-f {
  d: path("m41.86 62.07l3.69-34.29c.11-1.01.96-1.77 1.98-1.77h36.49c1.1 0 1.99.89 1.99 1.99v6.84c0 1.1-.89 1.99-1.99 1.99h-26.1c-1.01 0-1.86.76-1.97 1.76L54.5 51.21c-.17 1.49 1.29 2.6 2.69 2.07c2.65-1.01 5.54-1.52 8.64-1.52c7.14 0 12.69 2.28 16.66 6.85c3.96 4.57 5.95 10.71 5.95 18.43c0 7.62-2.2 13.69-6.59 18.2c-4.4 4.5-10.43 6.76-18.08 6.76c-6.87 0-12.56-1.95-17.07-5.84c-3.97-3.43-6.34-7.81-7.11-13.17c-.17-1.19.77-2.26 1.98-2.26h8.23c.94 0 1.74.66 1.94 1.58c.62 2.88 1.83 5.14 3.64 6.79c2.13 1.94 4.91 2.91 8.34 2.91c3.84 0 6.84-1.37 8.99-4.12c2.14-2.75 3.22-6.49 3.22-11.22c0-4.56-1.18-8.18-3.55-10.84s-5.63-3.99-9.78-3.99c-2.27 0-4.19.3-5.77.9c-1.35.52-2.81 1.45-4.36 2.81c-.5.44-1.17.62-1.81.46l-7.27-1.8a1.992 1.992 0 0 1-1.53-2.14z");
}

.vlzxxcccm {
  stop-color: var(--svg-color--757575, #757575);
}

</style>
