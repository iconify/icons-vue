<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="gt4a9dbai"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:fire-extinguisher-rounded" /></template>
<style>
.gt4a9dbai {
  fill: currentColor;
  d: path("M6.975 16v-3h10v3zM11.95 5.975q.425 0 .713-.288t.287-.712t-.287-.712t-.713-.288t-.712.288t-.288.712t.288.713t.712.287M7.05 11q.275-1.25 1.088-2.212t1.987-1.413q-.275-.225-.5-.488T9.25 6.3l-3.875-.75Q5.2 5.525 5.1 5.4T5 5.075V4.9q0-.2.1-.325t.275-.175l3.875-.725q.375-.775 1.1-1.237t1.6-.463q.6 0 1.113.213t.912.562L16.8 2.2q.475-.075.838.225T18 3.2v3.575q0 .475-.363.775t-.837.2l-2.825-.525q-.05.05-.087.075t-.088.075q1.175.45 1.988 1.413T16.875 11zm1.925 11q-.825 0-1.412-.587T6.975 20v-2h10v2q0 .825-.587 1.413T14.975 22z");
}

</style>
