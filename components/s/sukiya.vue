<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="kheidjmir"/><path class="aqo6jhhxo"/><path class="k5n60i5pb"/><path class="gv7xqt17n"/><path class="r981ojb8e"/><path class="u4oo19ymh"/><path class="bzjcm9bem"/><path class="kw9xsfbou"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:sukiya" /></template>
<style>
.aqo6jhhxo {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M15.5 12.921c3.027 7.98 3.598 12.505.88 16.327");
}

.bzjcm9bem {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M36.998 21.66c-.443.897-8.907 9.551-8.907 9.551");
}

.gv7xqt17n {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M22.182 19.27c.35.281 3.337 4.601 3.337 4.601c.396.518.665 1.619-.27 2.046s-4.733-.373-4.086 1.295s3.52 1.566 3.52 1.566M31.136 17.1l1.217 2.307m-4.352 1.054c.096.897 1.78 3.568 1.78 3.568");
}

.k5n60i5pb {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M17.235 20.845c1.158.923 1.158 2.847 0 4.297q0 0 0 0c-1.157 1.45-3.034 1.879-4.192.956q0 0 0 0c-1.158-.922-1.158-2.846 0-4.297q0 0 0 0c1.157-1.45 3.034-1.878 4.192-.956q0 0 0 0m3.562.377l3.587-1.273c.815-.435 1.54.975-3.47 4.364c0 0 4.733-2.4 5.429-2.979");
}

.kheidjmir {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M42.5 8.5V22c0 8.483-3.683 11.167-6.374 12.413l1.719 5.087h-27.69l1.72-5.087C9.182 33.167 5.5 30.483 5.5 22V8.5zM5.5 11h37M9.498 20.127l13.788-5.733");
}

.kw9xsfbou {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M32.665 21.384c3.652 4.05 1.829 11.101-1.385 4.144");
}

.r981ojb8e {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M28.341 21.443c.924-.702 6.637-3.237 6.91-3.317c.481-.14 4.255-1.434-.49 2.846");
}

.u4oo19ymh {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M30.771 22.122s3.206-1.456 3.788-1.476m-.842 2.236c.942.716 3.655 4.902 3.803 5.373m-4.542-6.569L29.468 25m4.249-2.118l-4.089 4.286");
}

</style>
