<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="jzr23vbvw"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:comment-off-20-filled" /></template>
<style>
.jzr23vbvw {
  fill: currentColor;
  d: path("M2.854 2.146a.5.5 0 1 0-.708.708l.763.762A2.55 2.55 0 0 0 2 5.566v6.71c0 1.418 1.164 2.566 2.6 2.566h.6v2.171c0 .213.07.42.2.591c.33.437.957.526 1.399.2l4.011-2.962h3.325l3.011 3.012a.5.5 0 0 0 .708-.708l-2.309-2.308L3.824 3.116zM18 12.276a2.56 2.56 0 0 1-1.351 2.251L5.12 3H15.4C16.836 3 18 4.149 18 5.566z");
}

</style>
