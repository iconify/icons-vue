<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="fipyqdvpb"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:fiare" /></template>
<style>
.fipyqdvpb {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M5.5 7.542v5.888l9.405 3.812L5.5 21.056v5.888l9.405 3.812L5.5 34.571v5.887l17.104-6.575v-6.247c-3.145-1.214-6.298-2.425-9.45-3.636l9.45-3.631v-6.248C16.91 11.924 11.2 9.733 5.5 7.541m37 .001v5.888l-9.405 3.812l9.405 3.814v5.888l-9.405 3.812l9.405 3.815v5.887l-17.104-6.575v-6.247c3.145-1.214 6.298-2.425 9.45-3.636l-9.45-3.631v-6.248C31.09 11.924 36.8 9.733 42.5 7.541");
}

</style>
