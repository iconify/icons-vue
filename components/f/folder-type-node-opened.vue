<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":32,"height":32};
const content = `<path class="v9erbbbht"/><path class="sawt_g-bm"/><path class="x3hdebb3q"/><path class="kqzryhbet"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="vscode-icons:folder-type-node-opened" /></template>
<style>
.kqzryhbet {
  fill: var(--svg-color--fff, #fff);
  d: path("M20.231 21.965a.22.22 0 0 1 .222-.222h1a.22.22 0 0 1 .221.188c.15 1.013.6 1.524 2.634 1.524c1.621 0 2.311-.367 2.311-1.227c0-.5-.2-.864-2.715-1.111c-2.106-.208-3.408-.674-3.408-2.358c0-1.553 1.309-2.479 3.5-2.479c2.466 0 3.685.856 3.84 2.691a.22.22 0 0 1-.221.242h-1a.22.22 0 0 1-.216-.174c-.239-1.066-.823-1.407-2.4-1.407c-1.77 0-1.975.617-1.975 1.079c0 .56.243.723 2.632 1.039c2.365.313 3.488.755 3.488 2.418s-1.4 2.637-3.836 2.637c-3.372.002-4.077-1.543-4.077-2.84");
}

.sawt_g-bm {
  fill: var(--svg-color--b1db7b, #b1db7b);
  d: path("M25.7 13.7H.5l3.8 12.8h25.2z");
}

.v9erbbbht {
  fill: var(--svg-color--b1db7b, #b1db7b);
  d: path("M27.4 5.5h-9.2l-2.1 4.2H4.3v16.8h25.2v-21Zm0 18.7H6.6V11.8h20.8Zm0-14.5h-8.2l1-2.1h7.1v2.1Z");
}

.x3hdebb3q {
  fill: var(--svg-color--fff, #fff);
  d: path("M21.688 31a1.6 1.6 0 0 1-.807-.216l-2.57-1.52c-.383-.215-.2-.291-.07-.335A5 5 0 0 0 19.4 28.4a.2.2 0 0 1 .191.015l1.974 1.172a.26.26 0 0 0 .239 0l7.7-4.442a.24.24 0 0 0 .118-.208v-8.881a.25.25 0 0 0-.119-.212l-7.692-4.438a.24.24 0 0 0-.238 0l-7.693 4.439a.25.25 0 0 0-.122.211v8.881a.24.24 0 0 0 .121.206l2.108 1.218c1.144.572 1.845-.1 1.845-.778v-8.769a.22.22 0 0 1 .223-.222h.976a.22.22 0 0 1 .222.222v8.769a2.132 2.132 0 0 1-2.278 2.4A3.3 3.3 0 0 1 15.2 27.5l-2.018-1.162a1.63 1.63 0 0 1-.807-1.4v-8.882a1.62 1.62 0 0 1 .807-1.4l7.7-4.447a1.69 1.69 0 0 1 1.617 0l7.7 4.447a1.62 1.62 0 0 1 .808 1.4v8.881a1.63 1.63 0 0 1-.808 1.4l-7.7 4.443a1.6 1.6 0 0 1-.811.22");
}

</style>
