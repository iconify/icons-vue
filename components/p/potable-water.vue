<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":64,"height":64};
const content = `<circle class="bgatx5byk"/><path class="ul9k_acdh"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="emojione:potable-water" /></template>
<style>
.bgatx5byk {
  cx: 32px;
  cy: 32px;
  r: 30px;
  fill: var(--svg-color--4fd1d9, #4fd1d9);
}

.ul9k_acdh {
  fill: var(--svg-color--fff, #fff);
  d: path("M36.6 24.1v-1H29v1c0 .6-.5 1-1 1h-5c-2.3 0-4.2 1.9-4.2 4.3v5.1h5.7v-1.8c0-1 .8-1.8 1.8-1.8h.6c.2 0 .5.2.6.4c1 1.9 3 3.1 5.3 3.1s4.2-1.3 5.3-3.2c.1-.2.4-.4.6-.4h4.8V25h-5.9c-.6.2-1-.3-1-.9m-12.1-7.4c.2 0 1.5 0 1.6-.1c.6 0 1.3-.1 4.4-.1c.1.2.3.3.5.5v2.3h-1c-.6 0-1 .5-1 1s.5 1 1 1h5.5c.6 0 1-.5 1-1s-.5-1-1-1h-1V17c.2-.1.3-.3.5-.5c3.1 0 3.8.1 4.4.1c.2 0 1.5.1 1.6.1c1 0 1.9-.8 1.9-1.9s-.8-1.9-1.9-1.9c-.2 0-1.5 0-1.6.1c-.6 0-1.3.1-4.4.1c-.5-.7-1.3-1.1-2.2-1.1s-1.7.4-2.2 1.1c-3.1 0-3.8-.1-4.4-.1c-.2 0-1.5-.1-1.6-.1c-1 0-1.9.8-1.9 1.9s.8 1.9 1.8 1.9m20.7 6.4H48v9.7h-2.8zM17.5 52h8.3l1.5-14H16zm8.4-9h-8.4l-.4-4h9.3z");
}

</style>
