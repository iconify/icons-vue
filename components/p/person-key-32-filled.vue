<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":32,"height":32};
const content = `<path class="quioi2yyw"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:person-key-32-filled" /></template>
<style>
.quioi2yyw {
  fill: currentColor;
  d: path("M16 16a7 7 0 1 0 0-14a7 7 0 0 0 0 14m-8.5 2A3.5 3.5 0 0 0 4 21.5v.5c0 2.393 1.523 4.417 3.685 5.793c1.83 1.165 4.203 1.932 6.815 2.146v-1.732a2 2 0 0 1 .586-1.414l4.536-4.536a6.5 6.5 0 0 1 .61-4.257zm22.036 6.535A5 5 0 0 1 25 25.9v.601a.5.5 0 0 1-.5.5h-1a.5.5 0 0 0-.5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 0-.5.5v1a.5.5 0 0 1-.5.5h-4a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l5.151-5.151a5 5 0 1 1 8.238 1.832m-2.829-4.242a1 1 0 1 0 1.414-1.414a1 1 0 0 0-1.414 1.414");
}

</style>
