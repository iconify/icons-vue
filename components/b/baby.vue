<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="drphzgq2g"/><path class="ned2hlb4k"/><path class="bv-_uacyf"/><path class="wb03m5bvy"/><circle class="i_fbu9d4w"/><circle class="w82x5gbbq"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:baby" /></template>
<style>
.bv-_uacyf {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M32.176 6.467c1.486-.359 2.767 1.05 2.738 1.972c-.03.922-.585 2.72-2.375 2.77s-2.045.244-4.117-.364c-.096-2.433 2.268-4.019 3.754-4.378m-23.37 5.717c2.393-2.531 3.63-3.07 5.923-2.687c2.292.383 2.66 2.13 2.596 2.573s-.502 1.435-2.062 1.698c-1.561.264-3.55.309-4.441-.2c-.892-.51-2.016-1.384-2.016-1.384m12.36 5.362c-1.016 1.673-1.637 3.363-.866 4.368c.772 1.005 1.754 1.122 2.532.612s1.323-1.54 2.004-1.386s.904.616 1.876.206c.973-.41.812-.059 1.703.191s2.295-.31 2.21-1.354c-.086-1.043-.35-1.625-1.162-3.14m-9.487 11.845c2.373-.992 3.69-1.898 4.733-1.947s1.801.617 2.477.311c.676-.305.847-1.048 1.683-.95s.92.666 1.682.926s-.651.271-1.222.748c-.572.477-1.697 1.207-2.64 1.418s-2.693.394-3.878.112s-2.835-.618-2.835-.618");
}

.drphzgq2g {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M38.062 5.5c.188 2 1.304 6.892 1.249 9.636c-.056 2.744-.023 7.001-.783 8.964s-1.676 3.727-2.34 4.352s-1.257 1.4-2.606 2.58c-1.35 1.181-3.798 2.305-4.67 4.013c-.488.958-2.093 1.976-2.96 2.362c-.51.227-1.487.207-2.447.33c-.668.086-1.305.347-1.866.367c-1.66.058-2.496-.18-3.365-.369c-1.02-.22-1.8-.728-4.251-.727c-2.451 0-5.335-1.605-5.335-1.605");
}

.i_fbu9d4w {
  cx: 30.555px;
  cy: 9.159px;
  r: 0.75px;
  fill: currentColor;
}

.ned2hlb4k {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  d: path("M23.065 37.932s1.024 2.239 1.78 4.568");
}

.w82x5gbbq {
  cx: 12.735px;
  cy: 11.174px;
  r: 0.75px;
  fill: currentColor;
}

.wb03m5bvy {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M10.25 10.953c-.027 1.728 1.146 2.649 2.465 2.675c1.319.027 2.357-.837 2.532-1.928s-.415-2.338-1.985-2.32M31.23 6.937c1.356.248 1.618 1.798 1.384 2.683c-.234.886-1.398 1.666-2.005 1.662");
}

</style>
