<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="ui065gb4p"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:nat-outline-sharp" /></template>
<style>
.ui065gb4p {
  fill: currentColor;
  d: path("M4 15q-1.25 0-2.125-.875T1 12t.875-2.125T4 9q.975 0 1.738.563T6.8 11H11v2H6.8q-.3.875-1.062 1.438T4 15m.713-2.287Q5 12.425 5 12t-.288-.712T4 11t-.712.288T3 12t.288.713T4 13t.713-.288M5 22v-2q3.325 0 5.663-2.337T13 12t-2.337-5.663T5 4V2q3.925 0 6.75 2.6t3.2 6.4h4.2L17.6 9.4L19 8l4 4l-4 4l-1.4-1.4l1.55-1.6h-4.2q-.375 3.8-3.2 6.4T5 22");
}

</style>
