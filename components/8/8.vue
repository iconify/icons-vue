<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":128,"height":128};
const content = `<linearGradient id="svgIDa" x1="64.01" x2="64.01" y1="101.51" y2="25.39" gradientUnits="userSpaceOnUse"><stop offset=".485" class="j1rdzyw_l"/><stop offset="1" class="vlzxxcccm"/></linearGradient><path fill="url(#svgIDa)" class="hmvhuccvo"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="noto:8" /></template>
<style>
.hmvhuccvo {
  d: path("M86.61 46.52c0 3.62-.93 6.85-2.79 9.68s-4.4 5.05-7.62 6.68c3.86 1.8 6.83 4.25 8.92 7.37c2.08 3.12 3.12 6.64 3.12 10.57c0 6.5-2.2 11.66-6.6 15.47s-10.26 5.72-17.58 5.72c-7.35 0-13.23-1.91-17.65-5.74s-6.63-8.97-6.63-15.44c0-3.96 1.05-7.52 3.15-10.67s5.05-5.57 8.84-7.27c-3.18-1.63-5.7-3.85-7.54-6.68c-1.85-2.83-2.77-6.05-2.77-9.68c0-6.3 2.03-11.29 6.1-14.99C51.6 27.85 57.09 26 64 26c6.94 0 12.45 1.85 16.51 5.54s6.1 8.68 6.1 14.98zM75.89 80.21c0-3.66-1.09-6.61-3.28-8.86s-5.07-3.38-8.66-3.38s-6.46 1.12-8.61 3.35c-2.15 2.24-3.23 5.2-3.23 8.89c0 3.62 1.06 6.52 3.18 8.69s5.04 3.25 8.76 3.25c3.73 0 6.63-1.05 8.71-3.15c2.09-2.1 3.13-5.03 3.13-8.79zm-1.58-33.23c0-3.22-.93-5.87-2.79-7.95s-4.37-3.12-7.52-3.12s-5.64.99-7.47 2.97s-2.74 4.68-2.74 8.1c0 3.39.92 6.08 2.77 8.08s4.34 3 7.49 3s5.65-1 7.49-3c1.85-2 2.77-4.69 2.77-8.08z");
}

.j1rdzyw_l {
  stop-color: var(--svg-color--504f4f, #504F4F);
}

.vlzxxcccm {
  stop-color: var(--svg-color--757575, #757575);
}

</style>
