<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="qx33kbtee"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:building-townhouse-24-regular" /></template>
<style>
.qx33kbtee {
  fill: currentColor;
  d: path("m8.039 4.577l2.952 2.558a.75.75 0 0 1 .259.567v7.797h-2.5a.75.75 0 0 0-.75.75v3.25H4.75a.25.25 0 0 1-.25-.25V7.702a.75.75 0 0 1 .259-.567L7.71 4.577a.25.25 0 0 1 .328 0m3.211 12.422v2.5H9.5V17zm1.5 0h1.75v2.5h-1.75zM16 19.5v-3.25a.75.75 0 0 0-.75-.75h-2.5V7.702a.75.75 0 0 1 .259-.567l2.952-2.558a.25.25 0 0 1 .328 0l2.952 2.558a.75.75 0 0 1 .259.567v11.547a.25.25 0 0 1-.25.25zM4.75 21h14.5A1.75 1.75 0 0 0 21 19.25V7.702a2.25 2.25 0 0 0-.777-1.7l-2.952-2.559a1.75 1.75 0 0 0-2.292 0l-2.952 2.558l-.027.024L11.973 6L9.021 3.443a1.75 1.75 0 0 0-2.292 0L3.777 6.001A2.25 2.25 0 0 0 3 7.701V19.25c0 .967.784 1.75 1.75 1.75M8.879 9a1 1 0 1 1-2 0a1 1 0 0 1 2 0m-1 4.997a1 1 0 1 0 0-2a1 1 0 0 0 0 2M16.121 10a1 1 0 1 1 0-2a1 1 0 0 1 0 2m-1 2.997a1 1 0 1 0 2 0a1 1 0 0 0-2 0");
}

</style>
