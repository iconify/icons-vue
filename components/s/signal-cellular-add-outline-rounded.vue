<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="x0ap1y4xu"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:signal-cellular-add-outline-rounded" /></template>
<style>
.x0ap1y4xu {
  fill: currentColor;
  d: path("M18 19h-2q-.425 0-.712-.288T15 18t.288-.712T16 17h2v-2q0-.425.288-.712T19 14t.713.288T20 15v2h2q.425 0 .713.288T23 18t-.288.713T22 19h-2v2q0 .425-.288.713T19 22t-.712-.288T18 21zM4.425 22q-.45 0-.725-.3t-.275-.7q0-.2.063-.375T3.7 20.3L20.3 3.725q.15-.15.325-.225T21 3.425q.4 0 .7.275t.3.725V11q0 .425-.288.713T21 12t-.712-.288T20 11V6.85L6.825 20H12q.425 0 .713.288T13 21t-.288.713T12 22zm2.4-2L20 6.85q-1.9 1.9-3.45 3.438t-3.037 3.024L10.425 16.4z");
}

</style>
