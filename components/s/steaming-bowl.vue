<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":36,"height":36};
const content = `<path class="h65gexbqr"/><path class="hoydlibhl"/><path class="qrdxyibqz"/><ellipse class="gio9bsbyv"/><path class="p5-go-c_c"/><path class="tuwknxs3o"/><path class="elun2cbjo"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="twemoji:steaming-bowl" /></template>
<style>
.elun2cbjo {
  fill: var(--svg-color--ffd983, #ffd983);
  d: path("M14 15a1 1 0 0 1-1-1V2a1 1 0 0 1 2 0v12a1 1 0 0 1-1 1m4 2a1 1 0 0 1-1-1V1a1 1 0 0 1 2 0v15a1 1 0 0 1-1 1");
}

.gio9bsbyv {
  cx: 18px;
  cy: 18px;
  fill: var(--svg-color--ffac33, #ffac33);
  rx: 15px;
  ry: 7px;
}

.h65gexbqr {
  fill: var(--svg-color--ccd6dd, #ccd6dd);
  d: path("M18 36C3.042 36 1 22 1 18h34c0 2-1.958 18-17 18");
}

.hoydlibhl {
  fill: var(--svg-color--99aab5, #99aab5);
  d: path("M35 17.056c0 5.216-7.611 9.444-17 9.444S1 22.271 1 17.056s7.611-8.444 17-8.444c9.389-.001 17 3.228 17 8.444");
}

.p5-go-c_c {
  fill: var(--svg-color--ffd983, #ffd983);
  d: path("M22 24c-1.18 0-2.122-.196-2.801-.584c-.767-.438-1.208-1.143-1.208-1.933C17.992 19.628 20.332 18 23 18a1 1 0 1 1 0 2c-1.872 0-3.008 1.03-3.008 1.483c0 .076.152.169.199.196c.21.12.724.321 1.809.321c2.404 0 5-1.529 5-4c0-.721-.94-1.183-1.343-1.348c-2.02-.826-5.291-.579-6.944 1.104c-1.441 1.467-3.792 1.956-6.29 1.301c-1.538-.402-3.319-1.338-3.705-2.761c-.196-.725.008-1.436.575-2.004C11.586 12 15.871 12 19 12a1 1 0 1 1 0 2c-2.779 0-6.586 0-8.293 1.707a.3.3 0 0 0-.064.082c.023.157.571.764 1.807 1.189c1.571.541 3.676.559 4.837-.623c2.319-2.361 6.461-2.644 9.128-1.554C28.058 15.473 29 16.639 29 18c0 3.706-3.634 6-7 6m-7 0c-7.605 0-10-5.456-10-7a1 1 0 0 1 2-.005C7.036 17.658 8.701 22 15 22a1 1 0 1 1 0 2");
}

.qrdxyibqz {
  fill: var(--svg-color--ea596e, #ea596e);
  d: path("M19.5 33H17a.5.5 0 0 1 0-1h2v-3h-4v1h3a.5.5 0 0 1 0 1h-3.5a.5.5 0 0 1-.5-.5v-2a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-.5.5m5.5-1a.5.5 0 0 1-.186-.964l2.176-.871l-.065-2.249l-3.925.975v.949a246 246 0 0 0 2.858-.819a.5.5 0 1 1 .283.959a317 317 0 0 1-3.507 1.002a.5.5 0 0 1-.438-.083A.5.5 0 0 1 22 30.5v-2a.5.5 0 0 1 .38-.485l4.906-1.219a.502.502 0 0 1 .62.471L28 30.485a.5.5 0 0 1-.315.479l-2.5 1A.5.5 0 0 1 25 32m4.573-2.712a.57.57 0 0 1-.288-.078a.62.62 0 0 1-.285-.491v-2.562a.5.5 0 0 1 .266-.441c.59-.313 1.279-.726 1.838-1.06c.39-.233.718-.43.911-.533a.5.5 0 0 1 .734.403c.034.438-.113 1.545-.144 1.763a.5.5 0 0 1-.247.366l-1.203.688a.506.506 0 0 1-.683-.186a.5.5 0 0 1 .186-.683l.987-.564c.018-.14.035-.293.052-.444l-.08.048c-.49.293-1.078.646-1.617.942v1.625c.173-.094.407-.229.724-.427l.339.379l.19.471c-.949.587-1.392.784-1.68.784m-17.698 2.993a.5.5 0 0 1-.152-.023l-3.219-1.031a.5.5 0 0 1-.324-.629a.503.503 0 0 1 .628-.324l2.585.828l.065-2.356l-2.856-.836l-.026.735l1.585.537a.5.5 0 0 1-.321.947l-1.938-.656a.5.5 0 0 1-.339-.491l.062-1.75a.5.5 0 0 1 .64-.462l3.844 1.125a.5.5 0 0 1 .359.493l-.094 3.406a.5.5 0 0 1-.499.487M5.844 29.5a.5.5 0 0 1-.262-.074l-1.375-.844a.5.5 0 0 1-.165-.688a.5.5 0 0 1 .688-.164l.608.373l-.008-1.623L4 25.73v.425l.743.448a.5.5 0 0 1-.517.856l-.984-.594A.5.5 0 0 1 3 26.438v-1.562a.5.5 0 0 1 .746-.436l2.328 1.312a.5.5 0 0 1 .254.433l.016 2.812a.5.5 0 0 1-.5.503");
}

.tuwknxs3o {
  fill: var(--svg-color--aa695b, #aa695b);
  d: path("m35.092 3.053l-23.005.886a1 1 0 1 1-.179-1.992L34.824.066a1 1 0 0 1 1.086.906c.048.549-.267 2.031-.818 2.081M35 8L12 7a1 1 0 0 1 0-2h23c.553 0 1 .448 1 1s-.447 2-1 2");
}

</style>
