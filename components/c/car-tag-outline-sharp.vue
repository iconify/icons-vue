<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="p_6fb_b_s"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:car-tag-outline-sharp" /></template>
<style>
.p_6fb_b_s {
  fill: currentColor;
  d: path("M6.428 15.692q.466 0 .788-.326q.323-.327.323-.793q0-.467-.327-.79q-.327-.321-.793-.321q-.467 0-.79.326q-.321.327-.321.794q0 .466.326.788q.327.322.794.322M4 17v-4.846zm.23 1v2H3v-7.846L5.14 6h3.976v1H5.889l-1.473 4.154h8.061l1 1H4V17h14v-1.521l1-1V20h-1.23v-2zM15 5.73q.31 0 .52-.21q.21-.209.21-.52t-.21-.52q-.209-.21-.52-.21t-.52.21q-.21.209-.21.52t.21.52q.209.21.52.21m2.6 7.203l-6.1-6.125V1.5h5.289l6.119 6.125zm0-1.422l3.887-3.886L16.362 2.5H12.5v3.887zm-2.023 4.181q.292 0 .532-.137q.239-.138.382-.369l-1.524-1.523q-.23.143-.368.382t-.137.532q0 .465.325.79t.79.325m1.42-8.696");
}

</style>
