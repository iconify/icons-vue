<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<circle class="nub49-b7h"/><path class="n1y2dpbwa"/><path class="qg1cqe_5x"/><circle class="pgqz9m8rf"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:smouldering-durtles" /></template>
<style>
.n1y2dpbwa {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M36.158 26.232c0 6.83-5.549 13.883-12.393 16.158c-5.613-1.953-12.392-9.112-12.392-15.941s5.548-12.583 12.392-12.583s12.393 5.537 12.393 12.366");
}

.nub49-b7h {
  cx: 22.115px;
  cy: 11.441px;
  r: 0.75px;
  fill: currentColor;
}

.pgqz9m8rf {
  cx: 25.832px;
  cy: 11.441px;
  r: 0.75px;
  fill: currentColor;
}

.qg1cqe_5x {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M21.07 21.237h5.467l2.896 4.949l-2.896 5.02H21.07l-2.788-5.02zm12.379-2.722l-5.464 5.196m1.448 2.475l6.725.046m-8.173 2.464l5.935 5.261m-10.116-2.75l-.038 11.183m-3.719-13.028l-6.128 4.781m4.364-7.957l-6.91.263m8.304-2.737l-5.414-5.291m9.541 2.816l-.038-7.37m7.454 2.486c5.913-.61 9.515 1.662 8.892 9.856l-4.296-2.872m-1.122 9.098c3.13 1.977 5.076 4.647 3.789 9.21c-3.46 1.87-6.25.337-8.749-2.688m-11.166.383c-2.79 1.455-4.87 4.213-9.262 2.74c-1.744-4.518 1.023-7.077 3.593-9.722m-1.156-8.932L8.22 25.788C6.386 16.4 11.138 15.79 16.353 16.4m11.756-1.753c1.068-12.3-9.485-12.073-8.766-.056");
}

</style>
