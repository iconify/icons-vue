<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="turtxccnl"/><path class="mfrlnku1c"/><path class="nacwa87qm"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:mondly-languages" /></template>
<style>
.mfrlnku1c {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M16.336 23.178c2.689.103 6.982 1.766 7.543 4.345c.41 1.89-5.047 3.926-4.795 7.991c-2.663-1.318-3-3.449-3.056-4.598s-2.13-2.692-1.935-4.094s1.178-2.972-.672-4.177c1.29.533 1.963.496 2.915.533");
}

.nacwa87qm {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M13.42 22.645c-1.471-.673-1.401-1.402-.967-2.089s1.64.28 2.426.294s2.671-2.817 3.782-3.378s2.19-1.08 1.643-2.229s-2.383-.757-3.14-1.234s-.968-1.99-1.332-2.102s-3.677 2.78-4.963 5.369c-1.051 2.117-2.145 3.687-1.135 4.794s1.864-.266 3.687.575Zm21.8-.841s2.944 3.266 3.434 1.36c1.6-6.22-5.397-11.23-6.252-11.145s-2.874.672-3.07 1.387s1.472 1.192 1.514 1.739s-1.935.897-3.028 1.304s-1.276 1.682-1.01 2.144s1.473-.574 2.356-.616s1.948.518 2.803.546s.954-1.009 1.85-.7s.463 1.794-1.008 2.214s-2.117-.553-3.52-.532c-1.815.027-3.924 1.009-3.77 3.406s4.135 1.402 4.612 2.566s.322 4.64 1.164 5.86s2.621 1.331 3.392-.59s-.196-2.915.477-3.869s3.133-2.253 1.829-3.515M18.345 12.465c.48-.686 1.277-1.515 2.016-1.515s1.134.375 1.704.375s1.921-.29 3.167-.29s1.499.738.401 1.683s-2.513 1.689-3.297 1.689s-1.268-3.34-1.975-3.457");
}

.turtxccnl {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M24.076 6.187c-10.004 0-18.113 7.532-18.113 16.822c0 5.336 2.683 10.082 6.855 13.164v4.774a.68.68 0 0 0 1.004.598L18.5 39.01c1.758.528 3.629.82 5.576.82c10.004 0 18.114-7.531 18.114-16.822S34.08 6.187 24.075 6.187");
}

</style>
