<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="zugh7vbjc"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:dual-screen-20-filled" /></template>
<style>
.zugh7vbjc {
  fill: currentColor;
  d: path("M4 4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h5.5V4zm2 9.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1-.5-.5M10.5 4v12H16a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2zm2 9h1a.5.5 0 0 1 0 1h-1a.5.5 0 0 1 0-1");
}

</style>
