<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="ksnhw5bdp"/><path class="a32casbih"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:neural-cloud" /></template>
<style>
.a32casbih {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M27.52 20.074c0-.965 1.166-1.741 2.613-1.741h7.234c1.447 0 2.612.776 2.612 1.74m0 7.853c0 .965-1.165 1.741-2.612 1.741h-7.234c-1.447 0-2.612-.776-2.612-1.74v-4.42m-7.626-5.175v11.336L8.605 18.355v1.469m0 3.657v6.187");
}

.ksnhw5bdp {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M24 31.686H4.5V16.314H24zh19.5V16.314H24");
}

</style>
