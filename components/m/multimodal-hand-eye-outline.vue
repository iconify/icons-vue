<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="hza_orbfe"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:multimodal-hand-eye-outline" /></template>
<style>
.hza_orbfe {
  fill: currentColor;
  d: path("M7.825 23q-.6 0-1.15-.225t-.975-.65L.6 17l.825-.85q.35-.35.85-.475t1 0l1.725.5V8q0-.425.288-.712T6 7t.713.288T7 8v10.825l-2.45-.7L7.125 20.7q.15.15.325.225t.375.075H12q.825 0 1.413-.587T14 19v-4q0-.425.288-.712T15 14t.713.288T16 15v4q0 1.65-1.175 2.825T12 23zM8 16v-4q0-.425.288-.712T9 11t.713.288T10 12v4zm3 0v-3q0-.425.288-.712T12 12t.713.288T13 13v3zm1 5H7.125zm4-10q-2.275 0-4.2-1.2T9 6.5q.875-2.1 2.8-3.3T16 2t4.2 1.2T23 6.5q-.875 2.1-2.8 3.3T16 11m2.688-2.65Q19.95 7.7 20.75 6.5q-.8-1.2-2.062-1.85T16 4t-2.687.65T11.25 6.5q.8 1.2 2.063 1.85T16 9t2.688-.65m-3.75-.788Q14.5 7.126 14.5 6.5t.438-1.062T16 5t1.063.438T17.5 6.5t-.437 1.063T16 8t-1.062-.437");
}

</style>
