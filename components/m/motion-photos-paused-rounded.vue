<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="fv-24xbmk"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:motion-photos-paused-rounded" /></template>
<style>
.fv-24xbmk {
  fill: currentColor;
  d: path("M10.665 14.472q.143-.144.143-.356V9.885q0-.213-.144-.357t-.357-.143t-.356.143t-.143.357v4.23q0 .213.144.357t.356.144t.357-.144m3.384 0q.143-.144.143-.356V9.885q0-.213-.144-.357q-.143-.143-.356-.143t-.356.143t-.144.357v4.23q0 .213.144.357t.357.144t.356-.144M12.003 21q-1.866 0-3.51-.705q-1.643-.706-2.859-1.915t-1.925-2.843T3 12.039q0-.762.125-1.504q.125-.743.356-1.48q.067-.207.262-.296q.196-.09.378-.005q.196.085.272.299t.009.428q-.185.62-.294 1.253T4 12q0 3.35 2.325 5.675T12 20t5.675-2.325T20 12t-2.325-5.675T12 4q-.63 0-1.246.097t-1.22.291q-.213.068-.415-.015t-.3-.265t-.012-.369q.085-.185.274-.253q.673-.236 1.377-.361T11.888 3q1.886 0 3.544.701t2.893 1.926t1.955 2.866t.72 3.505t-.708 3.509t-1.924 2.859t-2.856 1.925t-3.509.709m-6.7-14.457q-.264-.264-.264-.62t.264-.62t.62-.264t.62.264t.265.62t-.265.62t-.62.265t-.62-.265M7.75 7.75Q9.5 6 12 6t4.25 1.75T18 12t-1.75 4.25T12 18t-4.25-1.75T6 12t1.75-4.25");
}

</style>
