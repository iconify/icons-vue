<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="ft5dv1b6b"><path clip-rule="evenodd" class="nl6p42_hc"/><path clip-rule="evenodd" class="egqcncb2a"/><path class="ml7lturcn"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="token-branded:blxm" /></template>
<style>
.egqcncb2a {
  fill: var(--svg-color--459b6e, #459b6e);
  fill-rule: evenodd;
  d: path("M12 20.47a8.47 8.47 0 1 0 0-16.94a8.47 8.47 0 0 0 0 16.94m0 .53a9 9 0 1 0 0-18a9 9 0 0 0 0 18");
}

.ft5dv1b6b {
  fill: none;
}

.ml7lturcn {
  fill: var(--svg-color--459b6e, #459b6e);
  d: path("M12.265 10.413v2.118L16.5 8.56c.175-.176.424-.636 0-1.06c-.423-.423-.884-.174-1.058 0zm-.53 2.88v-2.118L7.5 15.146c-.174.18-.423.635 0 1.059c.424.423.885.18 1.06 0z");
}

.nl6p42_hc {
  fill: var(--svg-color--203a49, #203a49);
  fill-rule: evenodd;
  d: path("M12 19.413a7.412 7.412 0 1 0 0-14.824a7.412 7.412 0 0 0 0 14.824m-.265-6.915v-2.117L8.56 7.469c-.175-.175-.636-.423-1.06 0c-.423.424-.174.884 0 1.059zm.53.826v-2.117l4.235 3.97c.175.175.424.636 0 1.06c-.424.423-.884.174-1.059 0zm-2.822-7.01a.53.53 0 0 0-.71.784l2.912 2.648a.53.53 0 0 0 .71 0l2.911-2.648a.53.53 0 1 0-.709-.783L12 8.639zm8.09 2.569a.53.53 0 0 0-.742.301l-.672 2.023h-1.737a.53.53 0 0 0-.397.878l1.853 2.118a.529.529 0 0 0 .794-.699l-1.085-1.239h.953a.53.53 0 0 0 .503-.36l.55-1.662c.16.201.27.434.27.699v1.588a.53.53 0 1 0 1.06 0v-1.588a2.17 2.17 0 0 0-.52-1.377a2.7 2.7 0 0 0-.83-.688zM6.33 8.978a.53.53 0 0 1 .752 0l2.647 2.647a.53.53 0 0 1 .016.73l-2.647 2.912a.53.53 0 0 1-.832-.063a8 8 0 0 1-.709-1.345a5 5 0 0 1-.44-1.858a4.6 4.6 0 0 1 1.218-3.018zm1.747 2.493l-1.329-1.328a3.7 3.7 0 0 0-.519 1.328zm-1.848 1.06c.064.296.17.613.297.92c.084.207.18.397.264.572l1.372-1.493z");
}

</style>
