<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="mckwxjb0v"/><path class="z45racbwa"/><path class="bdjp-obhn"/><path class="yfj07tbfz"/><circle class="qez5pexvn"/><path class="a7jb20pla"/><path class="qvnadev6l"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:bookwyrm" /></template>
<style>
.a7jb20pla {
  fill: none;
  stroke: currentColor;
  d: path("M20.102 15.439c-.875-.023-2.367.188-2.787 2.524m-.503 2.997c-.761 4.701-1.889 12.08-1.889 12.08a2.23 2.23 0 0 0 .202 1.374");
}

.bdjp-obhn {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M32.854 26.676s1.798 2.237 3.558 1.744s3.438-2.318 3.872.655M12.24 16.724s2.944 2.035 5.108 1.469s6.619-2.712 8.648 2.17");
}

.mckwxjb0v {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M31.074 10.459c-4.79-2.117-3.72-3.134-10.746-5.48S5.184 11.195 8.2 19.175a10.5 10.5 0 0 0 7.276 6.924m16.96 2.374c2.069 2.909 1.726 6.986-2.677 8.295m-12.84 1.259c3.241 1.965 8.035 4.553 9.951 4.81c3.707.495 10.11-1.846 11.621-8.12a11.7 11.7 0 0 0-.4-6.914m-.858-1.809a9.2 9.2 0 0 0-3.342-3.497m-18.81 9.183a9.1 9.1 0 0 0-4.078-1.374c-5.133-.09-8.094 5.092-5.61 9.981s9.75 4.003 11.527-2.26s6.36-7.293 6.36-7.293s-5.215-1.367-8.084 3.314c0 0-2.895 6.878-5.412 4.741c-2.745-2.332.502-7.039 4.25-3.22");
}

.qez5pexvn {
  cx: 36.072px;
  cy: 12.92px;
  r: 0.75px;
  fill: currentColor;
}

.qvnadev6l {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  d: path("M41.414 28.874a4 4 0 0 0-.202-.97");
}

.yfj07tbfz {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M11.814 20.436a4.24 4.24 0 0 0 3.971.85c2.911-.757 6.346-3.624 7.664.263M24 19.698a3.4 3.4 0 0 1 .839 1.522m13.998-8.227a.84.84 0 0 1-1-.551a2.27 2.27 0 0 0-2.172-1.45");
}

.z45racbwa {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M13.058 16.755s-.982-5.58 3.554-6.507a8.76 8.76 0 0 1 7.165 1.282s-.373 1.712-1.82 1.875a2.65 2.65 0 0 0 3.024-.384a2.46 2.46 0 0 1-.564 2.25s1.777-.007 2.371-.642m-.175-.992s2.025 4.184 4.948 4.093s8.282-3.17 7.934 1.204l1.187-1.417l.692 1.676l.201-2.327l.975.16l-.027-1.341a10.05 10.05 0 0 1-5.067-.293s1.263-1.9 4.976-.973c0 0 1.657-.28.842-1.634s-1.571.659-1.571.659a18 18 0 0 1-2-1.464a6.8 6.8 0 0 1-2.724-3.475l-.78 1.582a1.54 1.54 0 0 0-2.21 1.216s-2.658-.66-2.756-2.052a5.5 5.5 0 0 0-.018 2.551s-2.097.013-2.502-.886c0 0 .037 3.754.916 4.111a3.67 3.67 0 0 1-3.016-1.39m4.716 20.853l2.775-16.556M20.292 32.523l11.037 1.967M20.102 15.439l9.391 1.673m2.937.523l1.674.298m-4.89 21.047l.784-4.674M17.404 37l11.81 1.98m4.066-14.544s1.804 2.24 3.121 1.955s5.72-2.914 6.02 2.048");
}

</style>
