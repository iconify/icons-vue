<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="lfj17jbsn"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:hr-resting-outline" /></template>
<style>
.lfj17jbsn {
  fill: currentColor;
  d: path("M3 21v-2h18v2zm2.425-9.6q-.725-.675-1.062-1.6t-.338-1.9q0-2.025 1.388-3.463T8.8 3q.9 0 1.725.325t1.475.95q.65-.625 1.475-.95T15.2 3q2.025 0 3.413 1.45T20 7.925q0 .975-.363 1.863T18.6 11.375l-5.225 5.075q-.275.275-.625.413T12 17t-.75-.137t-.625-.413zm1.4-1.4L12 15l5.2-5.025q.425-.4.625-.938t.2-1.112q0-1.2-.812-2.05t-1.988-.85q-.55 0-1.062.213t-.888.612l-.55.55q-.15.15-.337.225T12 6.7t-.375-.075T11.3 6.4l-.55-.55q-.4-.4-.888-.625T8.8 5q-1.175 0-1.987.85T6 7.9q0 .575.2 1.125t.625.975M12 10");
}

</style>
