<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="rt32pbhgz"/><path class="o57c48are"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:dekd-writer" /></template>
<style>
.o57c48are {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M42.5 25.449V38.5a4 4 0 0 1-4 4H21.171m-8.748 0H9.5a4 4 0 0 1-4-4v-29a4 4 0 0 1 4-4h29a4 4 0 0 1 4 4v7.832");
}

.rt32pbhgz {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M25.527 35.414c-1.285 1.574-1.821 3.314-.939 5.396c2.624-.05 4.006-1.301 5.291-2.875l6.678-16.033zm15.382-10.991l2.537-3.108c-.796-1.75-2.354-2.788-4.352-2.521l-2.537 3.108c1.81.362 3.486 1.164 4.352 2.521l-11.03 13.512m-16.125-2.06c-.485 2.641-.41 3.608 1.157 6.592c.153-1.494.077-2.063.642-2.954c.993 1.162 1.95 2.138 3.662 2.12c-2.292-.943-1.438-2.263-1.286-5.759m-1.893-7.554l2.718-16.85h4.8c5.154 0 8.8 3.301 8.143 7.372l-.34 2.106C30.7 25.02 25.99 28.32 20.836 28.32zm8.118 5.413l-14.485.003c-2.386 0-4.169 1.65-4.169 3.712m37-9.477c0 3.192-2.57 5.76-5.76 5.76h-.777");
}

</style>
