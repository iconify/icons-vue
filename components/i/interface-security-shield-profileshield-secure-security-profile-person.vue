<script setup>
import { Icon } from '@iconify/css-vue';
import '../../css/y/y9tr6bcfx.css';
import '../../css/z/zqgxwn5df.css';
import '../../css/f/f9bv6mbbv.css';
import '../../css/m/mt1nnt_ai.css';

const props = defineProps(["width","height"]);

const viewBox = {"width":14,"height":14};
const content = `<g class="y9tr6bcfx"><path class="zqgxwn5df"/><circle class="f9bv6mbbv"/><path class="mt1nnt_ai"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="streamline:interface-security-shield-profileshield-secure-security-profile-person" /></template>
