<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="m-eo3pbef"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:pickleball-outline-sharp" /></template>
<style>
.m-eo3pbef {
  fill: currentColor;
  d: path("M7.075 14.475q.475.475 1.05.7t1.2.225t1.2-.225t1.05-.7l.9-.9q.475-.475.7-1.05t.225-1.2t-.225-1.187t-.7-1.038l-3.8-3.8q-.3-.3-.712-.3t-.713.3L3.3 9.275q-.3.3-.3.7t.3.7zM18.575 22L12.7 16.125q-.725.65-1.612.95t-1.788.3q-1 0-1.937-.375t-1.688-1.125l-3.8-3.775q-.425-.425-.65-.987T1 9.975t.225-1.137t.65-.988L5.85 3.875q.425-.425.988-.65T7.974 3t1.138.225t.987.65l3.775 3.8q.75.75 1.125 1.688t.375 1.937q0 .9-.312 1.788T14.1 14.7l5.9 5.9zM19.5 9q-1.45 0-2.475-1.025T16 5.5t1.025-2.475T19.5 2t2.475 1.025T23 5.5t-1.025 2.475T19.5 9m0-2q.625 0 1.063-.437T21 5.5t-.437-1.062T19.5 4t-1.062.438T18 5.5t.438 1.063T19.5 7M8.15 10.15");
}

</style>
