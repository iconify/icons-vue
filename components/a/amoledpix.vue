<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="ivmcuyb7d"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:amoledpix" /></template>
<style>
.ivmcuyb7d {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M19.003 6.214S6.665 38.312 6.141 40.267C5.618 42.22 7.176 42.5 7.774 42.5h8.711c.396 0 .732.012 1.55-1.495c.758-1.398 5.99-18.076 5.99-18.076l5.86 18.076s.459 1.495 1.425 1.495h8.895c.58 0 2.168-.372 1.662-2.233c-.506-1.858-12.984-34.053-12.984-34.053s-.2-.714-1.484-.714h-6.867c-1.27 0-1.53.714-1.53.714z");
}

</style>
