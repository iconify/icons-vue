<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="tlyu1ebqs"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:meal-dinner-rounded" /></template>
<style>
.tlyu1ebqs {
  fill: currentColor;
  d: path("M8.05 14q-2.5 0-4.275-1.75T2 8q0-2.35 1.563-4.025T7.374 2.05q.325-.05.5.25t-.025.6q-.275.5-.4 1.013T7.325 5q0 1.875 1.312 3.188T11.826 9.5q.3 0 .6-.038T13 9.35q.35-.1.588.125t.137.525q-.65 1.775-2.187 2.888T8.05 14M18 11h2V8h-2zM2 22q-.425 0-.712-.288T1 21t.288-.712T2 20h1q-.025-.075-.038-.137t-.037-.138L2.3 17.25q-.125-.475.175-.862t.8-.388h9.45q.5 0 .8.388t.175.862l-.625 2.475q-.025.075-.037.138T13 20h5v-3.175q-.9-.325-1.45-1.1T16 14V7q0-.425.288-.712T17 6h4q.425 0 .713.288T22 7v7q0 .95-.55 1.725t-1.45 1.1V20h2q.425 0 .713.288T23 21t-.288.713T22 22z");
}

</style>
