<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="liz1a2b9c"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:shopping-bag-tag-48-filled" /></template>
<style>
.liz1a2b9c {
  fill: currentColor;
  d: path("M14 10a6 6 0 0 1 10-4.472A6 6 0 0 1 34 10v4h3.5a2.5 2.5 0 0 1 2.5 2.5v8.338l-1.079-1.079A6 6 0 0 0 34.674 22H29.5v-8h2v-4a3.5 3.5 0 0 0-6.014-2.434A6 6 0 0 1 26 10v4h1v8.084a6.01 6.01 0 0 0-5 5.923v6.667c0 1.593.633 3.121 1.76 4.247L28.837 44H14.5A6.5 6.5 0 0 1 8 37.5v-21a2.5 2.5 0 0 1 2.5-2.5H14zm9.5 4v-4a3.5 3.5 0 1 0-7 0v4zm.5 20.674v-6.667A4.007 4.007 0 0 1 28.007 24h6.667c1.063 0 2.082.422 2.833 1.174l7.32 7.319a4.006 4.006 0 0 1 0 5.666l-6.668 6.668a4.006 4.006 0 0 1-5.666 0l-7.32-7.32A4 4 0 0 1 24 34.674m4-5.174a1.5 1.5 0 1 0 3 0a1.5 1.5 0 0 0-3 0");
}

</style>
