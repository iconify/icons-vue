<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="v7y1_kftt"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:metabolism-outline-sharp" /></template>
<style>
.v7y1_kftt {
  fill: currentColor;
  d: path("M10.058 17.077v-3.116H8.75v-2.907q0-.46.634-.776q.633-.316 1.616-.316t1.616.316q.634.316.634.776v2.908h-1.308v3.115zm.098-8.117q-.348-.348-.348-.844q0-.497.348-.845T11 6.923t.844.348t.348.845t-.348.844t-.844.348t-.844-.348M11 20q-3.327 0-5.663-2.336T3 12q0-1.664.626-3.118T5.34 6.34t2.542-1.714T11 4t3.118.626T16.66 6.34t1.714 2.542T19 12v1.412l2.152-2.178l.714.689l-3.366 3.365l-3.365-3.365l.713-.688L18 13.387V12q0-2.9-2.05-4.95T11 5T6.05 7.05T4 12q.006 2.92 2.043 4.96T11 19q1.406 0 2.589-.499t2.17-1.386l.733.714q-1.129 1.027-2.539 1.599T11 20");
}

</style>
