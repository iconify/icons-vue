<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="e-nxe-xeb"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:flaky-outline-sharp" /></template>
<style>
.e-nxe-xeb {
  fill: currentColor;
  d: path("M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22M7.35 11.225L8.75 9.8l1.4 1.425l1.075-1.075L9.8 8.75l1.425-1.4l-1.075-1.075L8.75 7.7l-1.4-1.425L6.275 7.35L7.7 8.75l-1.425 1.4zM12 20q3.325 0 5.663-2.337T20 12q0-1.65-.625-3.1T17.65 6.35l-11.3 11.3q1.1 1.1 2.55 1.725T12 20m2.05-2.4l-2.4-2.4l1.05-1.075l1.35 1.35L16.55 13l1.05 1.05z");
}

</style>
