<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="jhgig_vbm"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:spatial-audio-sharp" /></template>
<style>
.jhgig_vbm {
  fill: currentColor;
  d: path("M23 10.025q-1.8 0-3.45-.687t-2.925-1.963t-1.963-2.925T13.976 1h2q0 1.425.525 2.7t1.525 2.275T20.3 7.513t2.7.537zm0-3.975q-1.025 0-1.937-.375t-1.638-1.1t-1.1-1.638T17.95 1h1.975q0 .625.238 1.188t.662.987t.988.65T23 4.05zM10 13q-1.65 0-2.825-1.175T6 9t1.175-2.825T10 5t2.825 1.175T14 9t-1.175 2.825T10 13m-8 8v-2.8q0-.825.425-1.55t1.175-1.1q1.275-.65 2.875-1.1T10 14t3.525.45t2.875 1.1q.75.375 1.175 1.1T18 18.2V21z");
}

</style>
