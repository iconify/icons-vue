<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="qeagbwb0v"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:bookmark-added-rounded" /></template>
<style>
.qeagbwb0v {
  fill: currentColor;
  d: path("M17.825 6.175L20.65 3.35q.3-.3.713-.3t.712.3t.3.713t-.3.712L18.525 8.3q-.3.3-.7.3t-.7-.3L15.7 6.875q-.275-.275-.275-.687t.275-.713q.3-.3.713-.3t.712.3zM12 18l-4.2 1.8q-1 .425-1.9-.162T5 17.975V5q0-.825.588-1.412T7 3h5.625q.45 0 .675.4t.025.825q-.175.425-.25.85T13 6q0 1.8 1.138 3.175T17 10.9q.3.05.538.063t.462.012q.425 0 .713.263T19 11.9v6.075q0 1.075-.9 1.663t-1.9.162z");
}

</style>
