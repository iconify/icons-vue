<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="ft5dv1b6b"><path fill="url(#SVGcLuxpdEj)" clip-rule="evenodd" class="m630wmbwc"/><defs><linearGradient id="SVGcLuxpdEj" x1="12" x2="12" y1="6.371" y2="17.63" gradientUnits="userSpaceOnUse"><stop class="doc5tobgq"/><stop offset="1" class="g-o08bk5b"/></linearGradient></defs></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="token-branded:vext" /></template>
<style>
.doc5tobgq {
  stop-color: var(--svg-color--6564fe, #6564fe);
}

.ft5dv1b6b {
  fill: none;
}

.g-o08bk5b {
  stop-color: var(--svg-color--4c40f8, #4c40f8);
}

.m630wmbwc {
  fill-rule: evenodd;
  d: path("m19.407 8.032l-5.139 5.274a.05.05 0 0 0-.013.031c0 .009 0 .023.009.031l.62.977l2.053 3.258c.013.022.009.031-.018.018L13.589 16c-.45-.225-.783-.36-1.003-.414a2.52 2.52 0 0 0-1.755.211L7.087 17.62q-.068.028-.027-.032l.4-.62l2.286-3.614a.04.04 0 0 0-.004-.045L8.54 12.09L3.028 6.422q-.067-.066.023-.045l8.293 2.385c.27.08.553.108.837.08a4 4 0 0 0 .751-.161q4.03-1.149 8.059-2.304l.006.003l.003.006V6.4l-1.593 1.628zm-6.592 1.89q2.573-.872 5.148-1.737c.013 0 .013.009 0 .018l-5.368 3.343a1.04 1.04 0 0 1-.788.158a2.3 2.3 0 0 1-.68-.324a646 646 0 0 0-5.025-3.141q-.17-.102.018-.04l5.417 1.84c.27.09.495.126.743.058q.115-.031.535-.18zm5.4-1.598q-.002-.018-.036-.022c-1.787 1.255-3.573 2.51-5.355 3.775c-.27.19-.472.297-.616.329a1.03 1.03 0 0 1-.828-.18q-2.82-1.991-5.643-3.978v.018l5.098 4.995q.014.013.014.031a.05.05 0 0 1-.01.032l-2.892 3.388q-.1.12.022.027c1.197-.869 2.34-1.71 3.424-2.511a.95.95 0 0 1 .801-.189q.224.046.603.328q1.645 1.217 3.303 2.417c.004 0 .009-.005.004-.014l-.004-.009l-2.947-3.446l-.007-.01l-.003-.013l.003-.012l.007-.01l5.057-4.941z");
}

</style>
