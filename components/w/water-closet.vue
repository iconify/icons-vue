<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":36,"height":36};
const content = `<path class="r9rioofmx"/><path class="gnv28abkz"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="twemoji:water-closet" /></template>
<style>
.gnv28abkz {
  fill: var(--svg-color--fff, #fff);
  d: path("M1.695 13.402a2.8 2.8 0 0 1-.088-.748c0-.704.594-1.452 1.562-1.452c1.057 0 1.562.616 1.761 1.584l1.87 9.352h.044l2.86-9.638c.242-.792.902-1.298 1.716-1.298s1.475.506 1.717 1.298l2.86 9.638h.044l1.87-9.352c.198-.968.704-1.584 1.76-1.584c.969 0 1.562.748 1.562 1.452c0 .308-.021.462-.088.748l-2.794 12.014c-.221.924-.946 1.716-2.179 1.716a2.244 2.244 0 0 1-2.178-1.606l-2.553-8.493h-.044l-2.553 8.493a2.244 2.244 0 0 1-2.178 1.606c-1.232 0-1.959-.792-2.179-1.716zm27.654-2.332c1.608 0 4.731.528 4.731 2.464c0 .792-.549 1.474-1.363 1.474c-.902 0-1.518-.77-3.367-.77c-2.729 0-4.136 2.31-4.136 4.994c0 2.619 1.431 4.863 4.136 4.863c1.85 0 2.598-.924 3.5-.924c.99 0 1.451.989 1.451 1.496c0 2.112-3.322 2.596-4.951 2.596c-4.466 0-7.568-3.542-7.568-8.097c-.002-4.576 3.078-8.096 7.567-8.096");
}

.r9rioofmx {
  fill: var(--svg-color--269, #269);
  d: path("M36 32a4 4 0 0 1-4 4H4a4 4 0 0 1-4-4V4a4 4 0 0 1 4-4h28a4 4 0 0 1 4 4z");
}

</style>
