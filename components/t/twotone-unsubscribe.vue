<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="bui_7sb5p"/><path class="dog7o_b7z"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="ic:twotone-unsubscribe" /></template>
<style>
.bui_7sb5p {
  fill: currentColor;
  d: path("M18.99 5H5l7 3.5zm.01 8.05V7l-7 3.5L5 7v8h10.35c.56-1.18 1.76-2 3.15-2c.17 0 .34.03.5.05");
  opacity: var(--svg-opacity--0-3, 0.3);
}

.dog7o_b7z {
  fill: currentColor;
  d: path("M20.99 14.04V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h10.05c.28 1.92 2.1 3.35 4.18 2.93c1.34-.27 2.43-1.37 2.7-2.71c.25-1.24-.16-2.39-.94-3.18m-2-9.04L12 8.5L5 5zm-3.64 10H5V7l7 3.5L19 7v6.05c-.16-.02-.33-.05-.5-.05c-1.39 0-2.59.82-3.15 2m5.15 2h-4v-1h4z");
}

</style>
