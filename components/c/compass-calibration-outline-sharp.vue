<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="xqo4ffbmc"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:compass-calibration-outline-sharp" /></template>
<style>
.xqo4ffbmc {
  fill: currentColor;
  d: path("M8.463 20.538Q7 19.075 7 17t1.463-3.537T12 12t3.538 1.463T17 17t-1.463 3.538T12 22t-3.537-1.463m5.662-1.412Q15 18.25 15 17t-.875-2.125T12 14t-2.125.875T9 17t.875 2.125T12 20t2.125-.875M7 12.15l-5-5q2-2 4.588-3.075T12 3t5.413 1.075T22 7.15l-5 5q-1.025-1.025-2.3-1.588T12 10t-2.7.563T7 12.15m.15-2.65q1.05-.75 2.288-1.125T12 8t2.538.363T16.85 9.45l2.2-2.2q-1.55-1.1-3.338-1.675T12 5t-3.713.575T4.95 7.25zM12 17");
}

</style>
