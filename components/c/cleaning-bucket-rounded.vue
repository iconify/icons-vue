<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="vc5qqfbrz"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:cleaning-bucket-rounded" /></template>
<style>
.vc5qqfbrz {
  fill: currentColor;
  d: path("M8.75 23q-.75 0-1.312-.488t-.663-1.237L5.15 9.125q-.05-.45.25-.787T6.15 8h11.7q.45 0 .75.338t.25.787l-1.625 12.15q-.1.75-.663 1.238T15.25 23zm5.375-7.875Q15 14.25 15 13v-1q0-.425-.288-.712T14 11t-.712.288T13 12v1q0 .425-.288.713T12 14t-.712-.288T11 13v-1q0-.425-.288-.712T10 11t-.712.288T9 12v1q0 1.25.875 2.125T12 16t2.125-.875M15 7q-.625 0-1.062-.437T13.5 5.5t.438-1.062T15 4t1.063.438T16.5 5.5t-.437 1.063T15 7m-5-1q-1.05 0-1.775-.725T7.5 3.5t.725-1.775T10 1t1.775.725T12.5 3.5t-.725 1.775T10 6");
}

</style>
