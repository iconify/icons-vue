<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="ft5dv1b6b"><path fill="url(#SVG6WQKhcBr)" clip-rule="evenodd" class="ymbqbf05k"/><defs><linearGradient id="SVG6WQKhcBr" x1="3" x2="21" y1="12" y2="12" gradientUnits="userSpaceOnUse"><stop class="h35nacbyr"/><stop offset="1" class="gb-x_zb_s"/></linearGradient></defs></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="token-branded:om" /></template>
<style>
.ft5dv1b6b {
  fill: none;
}

.gb-x_zb_s {
  stop-color: var(--svg-color--fe95ce, #fe95ce);
}

.h35nacbyr {
  stop-color: var(--svg-color--ffb298, #ffb298);
}

.ymbqbf05k {
  fill-rule: evenodd;
  d: path("m4.688 10.313l.374.36l-.375.356l-.375-.357zm-.312 1.368c.173.165.45.165.623 0L6 10.688l.375.28v2.72H6v-2.36l-.626.59a1 1 0 0 1-.686.27a1 1 0 0 1-.687-.27l-.626-.593v2.363H3v-2.72l.375-.28zm2.186 2.007l1.227-3h.551l1.223 3H9.15l-.315-.75H7.312l-.322.75zm1.485-2.625l-.61 1.5h1.267l-.634-1.5zM12 13.046h-.023l-1.784-2.358H9.75v3h.375v-2.43h.026L12 13.684l.375.004v-3H12zm.75-1.987v-.371h2.625v.375H14.25v2.624h-.375v-2.624H12.75zm3-.371h1.271c.638 0 .979.345.979.825v.06c0 .45-.206.57-.442.637v.026c.27.011.442.165.442.548v.903h-.375v-.858c0-.267-.293-.267-.525-.267h-.975v1.126h-.375zm2.25 3l1.226-3h.552l1.222 3h-.413l-.314-.75H18.75l-.323.75zm-1.875-1.5h.84c.375 0 .66-.29.66-.563c0-.274-.274-.562-.675-.562h-.825zm2.749.374l.611-1.5h.023l.633 1.5z");
}

</style>
