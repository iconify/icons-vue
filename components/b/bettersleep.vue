<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="g2wj9jbhp"/><path class="g-zts3bkw"/><path class="uou3x0bcp"/><path class="zme851i-x"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:bettersleep" /></template>
<style>
.g-zts3bkw {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M12.542 32.822a13.54 13.54 0 0 1-2.155-7.329a13.54 13.54 0 0 1 13.54-13.539a13.54 13.54 0 0 1 13.54 13.54v0a13.5 13.5 0 0 1-.616 4.034");
}

.g2wj9jbhp {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M9.5 5.5h29c2.216 0 4 1.784 4 4v29c0 2.216-1.784 4-4 4h-29c-2.216 0-4-1.784-4-4v-29c0-2.216 1.784-4 4-4");
}

.uou3x0bcp {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M5.89 35.248c8.845-3.646 24.483-7.259 36.224-4.981");
}

.zme851i-x {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M27.025 29.849c-1.29-1.027-2.082-4.488-1.09-3.796c.711.495 1.232 1.111 1.676 1.775c.259-2.698 1.64-4.906 1.671-3.487l.077 3.575c1.848-3.416 5.047-3.093 4.466-2.407l-2.94 3.468c-.129.152-.243.425-.167.593");
}

</style>
