<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="nsawmq2wv"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:quiz-rounded" /></template>
<style>
.nsawmq2wv {
  fill: currentColor;
  d: path("M14.045 14.303q.236-.236.236-.545t-.236-.545t-.545-.236t-.545.236t-.236.545t.236.545t.545.236t.545-.236m-.545-2.58q.179 0 .31-.123t.163-.294q.05-.396.232-.665q.181-.27.703-.752q.634-.577.884-1.03t.25-1.04q0-1.01-.72-1.683q-.72-.674-1.822-.674q-.71 0-1.308.336q-.598.337-.96.96q-.091.153-.024.323t.232.236q.16.068.336.016t.274-.206q.263-.402.621-.603t.829-.201q.715 0 1.186.424q.472.424.472 1.095q0 .408-.23.759q-.228.351-.786.845q-.57.49-.79.876t-.27.959q-.024.165.102.304q.125.138.316.138M8.116 17q-.691 0-1.153-.462T6.5 15.385V4.615q0-.69.463-1.153T8.116 3h10.769q.69 0 1.153.462t.462 1.153v10.77q0 .69-.462 1.152T18.884 17zm-3 3q-.691 0-1.153-.462T3.5 18.385V7.115q0-.213.143-.356T4 6.616t.357.143t.143.357v11.269q0 .23.192.423t.423.192h11.27q.213 0 .356.143t.144.357t-.144.357t-.356.143z");
}

</style>
