<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":32,"height":32};
const content = `<path class="fc8n9ab6g"/><path class="zmgxj_bkt"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="vscode-icons:file-type-node2" /></template>
<style>
.fc8n9ab6g {
  fill: var(--svg-color--83cd29, #83cd29);
  fill-rule: evenodd;
  d: path("M16 2A14 14 0 1 1 2 16A14 14 0 0 1 16 2");
}

.zmgxj_bkt {
  fill: var(--svg-color--fff, #fff);
  d: path("M16 30a2.15 2.15 0 0 1-1.076-.288L11.5 27.685c-.511-.286-.262-.387-.093-.446a6.8 6.8 0 0 0 1.549-.7a.26.26 0 0 1 .255.019l2.631 1.563a.34.34 0 0 0 .318 0l10.26-5.922a.32.32 0 0 0 .157-.278V10.075a.33.33 0 0 0-.159-.283l-10.26-5.917a.32.32 0 0 0-.317 0L5.587 9.794a.33.33 0 0 0-.162.281v11.841a.32.32 0 0 0 .161.274L8.4 23.814c1.525.762 2.459-.136 2.459-1.038V11.085a.3.3 0 0 1 .3-.3h1.3a.3.3 0 0 1 .3.3v11.692c0 2.035-1.108 3.2-3.038 3.2a4.4 4.4 0 0 1-2.363-.642l-2.697-1.547a2.17 2.17 0 0 1-1.076-1.872V10.075A2.16 2.16 0 0 1 4.661 8.2l10.261-5.924a2.25 2.25 0 0 1 2.156 0L27.338 8.2a2.17 2.17 0 0 1 1.077 1.87v11.846a2.17 2.17 0 0 1-1.077 1.872l-10.26 5.924A2.15 2.15 0 0 1 16 30m3.488-8.257c3.251 0 5.115-1.28 5.115-3.516c0-2.216-1.5-2.807-4.651-3.223c-3.186-.422-3.51-.639-3.51-1.385c0-.616.274-1.438 2.634-1.438c2.108 0 2.885.454 3.2 1.875a.3.3 0 0 0 .288.232H23.9a.3.3 0 0 0 .295-.323c-.206-2.448-1.832-3.589-5.12-3.589c-2.925 0-4.67 1.235-4.67 3.305c0 2.246 1.736 2.866 4.544 3.144c3.359.329 3.62.82 3.62 1.481c0 1.147-.92 1.636-3.082 1.636c-2.715 0-3.313-.682-3.513-2.032a.3.3 0 0 0-.295-.251h-1.328a.3.3 0 0 0-.3.3c.003 1.723.944 3.784 5.434 3.784Z");
}

</style>
