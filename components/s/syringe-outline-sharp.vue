<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="x1m-kvbgy"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:syringe-outline-sharp" /></template>
<style>
.x1m-kvbgy {
  fill: currentColor;
  d: path("m5.279 7.164l-.713-.708l2.713-2.733l.727.714l-1.02 1.019L8.37 6.819l2.852-2.852l.714.714l-.939.933l7.948 7.928l-1.907 1.92l4.032 4.027h-1.427l-3.319-3.32l-1.889 1.9l-7.953-7.934l-.914.907l-.688-.688l2.8-2.846l-1.382-1.364zm1.89 2.282l7.24 7.24l3.095-3.119l-1.846-1.89l-1.894 1.889l-.702-.67l1.907-1.907l-2.096-2.077l-1.87 1.863l-.707-.708l1.864-1.869l-1.871-1.871zm0 0l.443-.442l2.677-2.677z");
}

</style>
