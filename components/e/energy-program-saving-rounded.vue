<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="fcqwl5bsm"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:energy-program-saving-rounded" /></template>
<style>
.fcqwl5bsm {
  fill: currentColor;
  d: path("M11.5 16.5q1.667 0 2.833-1.167Q15.5 14.167 15.5 12.5v-4h-4q-1.667 0-2.833 1.167Q7.5 10.833 7.5 12.5q0 .69.194 1.328t.64 1.149l-.688.67q-.16.159-.16.353t.16.354t.354.16t.354-.16l.669-.688q.53.426 1.17.63q.638.204 1.307.204m.5 5.077l-3.004-2.35l-3.765-.458l-.457-3.765L2.423 12l2.35-3.004l.458-3.765l3.765-.457L12 2.423l3.004 2.35l3.765.458l.457 3.765L21.577 12l-2.35 3.004l-.458 3.765l-3.765.457zm.354-9.223q.16-.165.16-.357t-.16-.35q-.16-.16-.351-.16t-.357.16L9.4 13.891q-.165.166-.165.357q0 .192.165.357q.166.165.366.156q.201-.01.342-.156z");
}

</style>
