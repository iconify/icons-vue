<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="wh3ww1fic"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:foxit-pdf" /></template>
<style>
.wh3ww1fic {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M21.254 33.426H23.1c3.16-.31 6.285-1.441 8.458-3.617c2.023-2.019 3.699-5.008 3.92-8.071c.212-2.893-.619-4.385-1.997-5.845c-.18 1.491.017 2.887-.462 4.076c-.84-1.11-1.14-2.758-2.386-3.464c.049 4.34-2.396 7.536-6.996 6.231c.862 2.291 2.947 3.356 6.228 3.228c.156.178-.148.38-.23.462c-1.163 1.171-2.95 2.08-5.153 2.08c-4.116 0-6.798-2.851-6.841-6.921c-.06-5.355 4.841-9.614 9.841-9.921c2.686-.165 5.137 1.29 6.305 2.921c-.55-3.908-3.659-5.26-6.843-6.537c6.685-.3 9.665 3.1 11.687 7.46c-.93-6.147-4.353-10.464-11.152-10.996c-.717 0-8.23-.534-13.533 5.536c-2.654 3.04-4.915 7.013-4.535 12.302c.448 6.243 4.997 10.775 11.843 11.075zM15.03 43.5v-6.435h2.107c1.19 0 2.156.968 2.156 2.161s-.965 2.161-2.156 2.161H15.03m7.154 2.113v-6.435h1.448a2.815 2.815 0 0 1 2.815 2.816v.804a2.815 2.815 0 0 1-2.815 2.815zm7.274-3.217h2.091M29.458 43.5v-6.435h3.218");
}

</style>
