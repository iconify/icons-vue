<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":64,"height":64};
const content = `<g class="ic-371bqb"><path class="gvv297dne"/><path class="y3dhb8tzg"/></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="emojione:japanese-bargain-button" /></template>
<style>
.gvv297dne {
  d: path("M32 2C15.4 2 2 15.4 2 32s13.4 30 30 30s30-13.4 30-30S48.6 2 32 2m0 54C18.7 56 8 45.3 8 32S18.7 8 32 8s24 10.7 24 24s-10.7 24-24 24");
}

.ic-371bqb {
  fill: var(--svg-color--ff5a79, #ff5a79);
}

.y3dhb8tzg {
  d: path("M23.2 22.8c-1.9 3.3-5.2 6.6-8.2 8.8c.7 1.2 1.8 3.9 2 5c1.1-.9 2.1-1.8 3.2-2.9v15.1h4.4V28.6c1.1-1.4 2-2.8 2.8-4.2zm3.6-6l-4.2-1.7c-1.5 2.4-4.5 5.2-7 6.9c.6 1 1.6 2.9 2 3.9c3.1-2.1 6.7-5.8 9.2-9.1m17.5 19V34h3.8v-3.8H27.3V34h12.3v1.8H26.4v3.9H31l-2.5 1.6c1.5 1.5 3.3 3.6 4.1 5l3.4-2.4c-.7-1.2-2.2-2.8-3.6-4.2h7.2v4.5c0 .5-.2.6-.7.6s-2 0-3.5-.1c.6 1.2 1.2 2.9 1.4 4.1c2.6 0 4.4-.1 5.7-.7c1.4-.7 1.8-1.8 1.8-3.9v-4.6H49v-3.9zm3-19.6H28.8v12.5h18.6V16.2zm-4.9 9.5h-8.9v-1.8h8.9zm0-4.7h-8.9v-1.7h8.9z");
}

</style>
