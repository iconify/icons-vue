<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="sbzegfbsi"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:camera-video-outline-sharp" /></template>
<style>
.sbzegfbsi {
  fill: currentColor;
  d: path("M9.275 19h5.45l-.5-2h-4.45zM9 15h6q2.075 0 3.538-1.463T20 10t-1.463-3.537T15 5H9Q6.925 5 5.463 6.463T4 10t1.463 3.538T9 15m1.588-3.588Q10 10.826 10 10t.588-1.412T12 8t1.413.588T14 10t-.587 1.413T12 12t-1.412-.587M6.5 9q.425 0 .713-.288T7.5 8t-.288-.712T6.5 7t-.712.288T5.5 8t.288.713T6.5 9M4 21v-2h3.225l.525-2.1q-2.475-.425-4.112-2.363T2 10q0-2.925 2.038-4.962T9 3h6q2.925 0 4.963 2.038T22 10q0 2.6-1.638 4.538T16.25 16.9l.525 2.1H20v2zm10.825-8.175Q16 11.65 16 10t-1.175-2.825T12 6T9.175 7.175T8 10t1.175 2.825T12 14t2.825-1.175M9.275 19h5.45z");
}

</style>
