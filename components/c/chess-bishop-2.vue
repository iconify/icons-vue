<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="ae35afbdr"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:chess-bishop-2" /></template>
<style>
.ae35afbdr {
  fill: currentColor;
  d: path("M6.616 17.289q-.737-.91-1.176-2.46Q5 13.279 5 11.636Q5 9.672 6.48 7.89t4.495-3.47q-.183-.181-.29-.438q-.108-.257-.108-.54q0-.594.414-1.018Q11.406 2 12 2t1.009.424t.414 1.018q0 .296-.108.538q-.107.241-.304.422q3.01 1.477 4.5 3.266Q19 9.458 19 11.618q0 1.713-.434 3.274q-.433 1.562-1.162 2.397zM5.962 21q-.414 0-.688-.284Q5 20.433 5 20.02t.284-.687t.697-.274h12.058q.413 0 .687.274t.274.687t-.274.697t-.687.284zm5.538-7.096h1V12h1.846v-1H12.5V9.116h-1V11H9.634v1H11.5z");
}

</style>
