<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="kconyy7hq"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:folder-copy-outline-rounded" /></template>
<style>
.kconyy7hq {
  fill: currentColor;
  d: path("M3.616 19.77q-.672 0-1.144-.473Q2 18.825 2 18.154V7.577q0-.213.143-.357t.357-.143t.357.143t.143.357v10.577q0 .269.173.442t.443.173H18.5q.214 0 .357.144t.143.356t-.143.357t-.357.143zm3-3q-.672 0-1.144-.473Q5 15.825 5 15.154V4.616q0-.672.472-1.144T6.616 3h4.29q.323 0 .628.13q.305.132.522.349L13.577 5h6.808q.67 0 1.143.472q.472.472.472 1.144v8.538q0 .671-.472 1.143t-1.144.472zm0-1h13.769q.269 0 .442-.174q.173-.173.173-.442V6.616q0-.27-.173-.443T20.385 6h-7.21l-2-2h-4.56q-.269 0-.442.173T6 4.616v10.538q0 .269.173.442t.443.173m-.616 0V4z");
}

</style>
