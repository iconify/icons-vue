<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="uwr63ptmt"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:everyday-rewards" /></template>
<style>
.uwr63ptmt {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M30.12 4.5h7.695c.824 0 .934.724.934 1.298v6.461c0 4.169-4.397 7.525-9.858 7.525h-5.783c.773-4.692 3.056-7.734 9.775-9.195c.474-.103.688-.939 0-.916c-6.29.21-12.608 5.141-12.608 9.005l.002 22.977a1.84 1.84 0 0 1-1.849 1.845H11.1a1.843 1.843 0 0 1-1.849-1.845V6.345c0-1.022.825-1.845 1.849-1.845h7.328c1.024 0 1.849.823 1.849 1.845v5.291C20.538 7.65 24.83 4.5 30.12 4.5");
}

</style>
