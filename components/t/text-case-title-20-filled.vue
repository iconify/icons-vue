<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":20,"height":20};
const content = `<path class="vzduf_b1f"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:text-case-title-20-filled" /></template>
<style>
.vzduf_b1f {
  fill: currentColor;
  d: path("M12.5 3.25a.75.75 0 0 1 .75.75v4.657A2.7 2.7 0 0 1 15 8c1.657 0 3 1.567 3 3.5S16.657 15 15 15a2.7 2.7 0 0 1-1.75-.657v.157a.75.75 0 0 1-1.5 0V4a.75.75 0 0 1 .75-.75M15 13.5c.62 0 1.5-.67 1.5-2s-.88-2-1.5-2s-1.5.67-1.5 2s.88 2 1.5 2M6.495 3.25a.75.75 0 0 1 .698.504l3.652 10.5a.75.75 0 1 1-1.417.492L8.647 12.5H4.06l-.86 2.266a.75.75 0 0 1-1.402-.532l3.984-10.5a.75.75 0 0 1 .712-.484M4.63 11h3.495L6.454 6.195z");
}

</style>
