<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="c0vbq-ben"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:image-split-24-filled" /></template>
<style>
.c0vbq-ben {
  fill: currentColor;
  d: path("M12.5 2.75a.75.75 0 0 0-1.5 0v18.5a.75.75 0 0 0 1.5 0zM5.25 4.5A1.75 1.75 0 0 0 3.5 6.25v.5a.75.75 0 0 1-1.5 0v-.5A3.25 3.25 0 0 1 5.25 3h.5a.75.75 0 0 1 0 1.5zm0 15a1.75 1.75 0 0 1-1.75-1.75v-.5a.75.75 0 0 0-1.5 0v.5A3.25 3.25 0 0 0 5.25 21h.5a.75.75 0 0 0 0-1.5zM7.5 3.75A.75.75 0 0 1 8.25 3h1a.75.75 0 0 1 0 1.5h-1a.75.75 0 0 1-.75-.75m.75 15.75a.75.75 0 0 0 0 1.5h1a.75.75 0 0 0 0-1.5zm-5.5-11a.75.75 0 0 1 .75.75v5.5a.75.75 0 0 1-1.5 0v-5.5a.75.75 0 0 1 .75-.75M13.5 21h5.25c.625 0 1.208-.176 1.703-.481l-6.928-6.801l-.025-.021zm0-8.94c.34.076.666.233.948.47l.128.118l6.939 6.811A3.24 3.24 0 0 0 22 17.75V6.25A3.25 3.25 0 0 0 18.75 3H13.5zm6.25-3.31a2 2 0 1 1-4 0a2 2 0 0 1 4 0");
}

</style>
