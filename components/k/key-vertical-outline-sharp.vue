<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="jryfptbyh"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:key-vertical-outline-sharp" /></template>
<style>
.jryfptbyh {
  fill: currentColor;
  d: path("M11.626 6.126q.357-.357.874-.357t.874.357t.357.874t-.357.874t-.874.357t-.874-.357T11.269 7t.357-.874m.874 16.663l-3.154-3.193l1.289-1.711l-1.327-1.808l1.192-1.779v-2.713q-1.311-.57-2.156-1.807T7.5 7q0-2.077 1.462-3.538T12.5 2t3.539 1.462T17.5 7q0 1.54-.835 2.778q-.834 1.238-2.165 1.807v9.204zM8.5 7q0 1.477.889 2.52t2.111 1.336v3.76l-.967 1.43l1.333 1.858l-1.222 1.621l1.856 1.856l1-1v-9.525q1.223-.292 2.112-1.336T16.5 7q0-1.65-1.175-2.825T12.5 3T9.675 4.175T8.5 7");
}

</style>
