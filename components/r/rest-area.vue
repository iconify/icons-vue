<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="mq7quvvzp"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:rest-area" /></template>
<style>
.mq7quvvzp {
  fill: currentColor;
  d: path("M5.827 21v-2H3.942v-1h4.77v1H6.827v2zm5.673 0v-5.384H7.712v-1h8.557v1H12.5V21zm5.673 0v-2h-1.904v-1h4.789v1h-1.885v2zM4.616 15.616q-.691 0-1.153-.463T3 14V4.616q0-.691.463-1.153T4.615 3h14.77q.69 0 1.152.463T21 4.616V14q0 .69-.462 1.153t-1.153.463h-.731v-1h.73q.27 0 .443-.174Q20 14.27 20 14v-1.756l-5.142-4.298l-3.794 3.795l-3.089-2.858L4 12.219V14q0 .27.173.442q.173.173.443.173h.711v1z");
}

</style>
