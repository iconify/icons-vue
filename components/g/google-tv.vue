<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="amyl2vmbi"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:google-tv" /></template>
<style>
.amyl2vmbi {
  fill: currentColor;
  d: path("M22 9.3q-.65-.6-1.475-.95Q19.7 8 18.75 8H7.5v-.75q0-1.35.95-2.3Q9.4 4 10.75 4H20q.825 0 1.413.588Q22 5.175 22 6ZM5.25 14.5q-1.35 0-2.3-.95Q2 12.6 2 11.25V6q0-.825.588-1.412Q3.175 4 4 4h3.3q-.6.65-.95 1.475Q6 6.3 6 7.25v7.25ZM4 20q-.825 0-1.412-.587Q2 18.825 2 18v-3.3q.65.6 1.475.95Q4.3 16 5.25 16H16.5v.75q0 1.35-.95 2.3q-.95.95-2.3.95Zm12.7 0q.6-.65.95-1.475q.35-.825.35-1.775V9.5h.75q1.35 0 2.3.95q.95.95.95 2.3V18q0 .825-.587 1.413Q20.825 20 20 20Z");
}

</style>
