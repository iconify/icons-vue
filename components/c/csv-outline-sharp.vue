<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="a59hrwbji"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:csv-outline-sharp" /></template>
<style>
.a59hrwbji {
  fill: currentColor;
  d: path("M4.75 15h4v-1.5h-2.5v-3h2.5V9h-4zm4.9 0h4v-3.65h-2.5v-.85h2.5V9h-4v3.6h2.5v.9h-2.5zm6.6 0h1.5l1.75-6H18l-1 3.45L16 9h-1.5zM2 20V4h20v16zm2-2h16V6H4zm0 0V6z");
}

</style>
