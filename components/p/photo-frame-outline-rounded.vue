<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="t_5ntutup"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:photo-frame-outline-rounded" /></template>
<style>
.t_5ntutup {
  fill: currentColor;
  d: path("M5.654 19.423q-.348 0-.578-.23t-.23-.577V18h-1.23q-.691 0-1.153-.462T2 16.384V6.616q0-.691.463-1.153T3.616 5h16.769q.69 0 1.153.463T22 6.616v9.769q0 .69-.462 1.153T20.385 18h-1.231v.616q0 .348-.23.577t-.578.23zM3.616 17h16.769q.23 0 .423-.192t.192-.424V6.616q0-.231-.192-.424T20.385 6H3.615q-.23 0-.423.192T3 6.616v9.769q0 .23.192.423t.423.192m3.622-2.5h9.57q.243 0 .355-.217t-.031-.43l-2.693-3.597q-.13-.162-.323-.171q-.192-.01-.323.151L11 13.828l-1.773-2.16q-.13-.142-.32-.139t-.32.164l-1.673 2.162q-.162.212-.044.429t.367.217M3 17V6z");
}

</style>
