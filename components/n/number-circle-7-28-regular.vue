<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":28,"height":28};
const content = `<path class="ffth6qwck"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="fluent:number-circle-7-28-regular" /></template>
<style>
.ffth6qwck {
  fill: currentColor;
  d: path("M3.5 14C3.5 8.201 8.201 3.5 14 3.5S24.5 8.201 24.5 14S19.799 24.5 14 24.5S3.5 19.799 3.5 14M14 2C7.373 2 2 7.373 2 14s5.373 12 12 12s12-5.373 12-12S20.627 2 14 2M9.75 8a.75.75 0 0 0 0 1.5h6.995a26 26 0 0 0-1.2 1.903c-1.13 1.976-2.419 4.807-3.033 8.214a.75.75 0 0 0 1.476.266c.576-3.193 1.788-5.862 2.86-7.736a24 24 0 0 1 1.394-2.165a16 16 0 0 1 .541-.7l.028-.034l.006-.006V9.24A.75.75 0 0 0 18.25 8z");
}

</style>
