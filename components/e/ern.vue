<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<g class="ft5dv1b6b"><path fill="url(#SVGZHIKHbHe)" clip-rule="evenodd" class="x66lllhpd"/><defs><linearGradient id="SVGZHIKHbHe" x1="21.585" x2="3.554" y1="11.998" y2="11.998" gradientUnits="userSpaceOnUse"><stop class="dkzpq1bkh"/><stop offset="1" class="ymshwgb8g"/></linearGradient></defs></g>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="token-branded:ern" /></template>
<style>
.dkzpq1bkh {
  stop-color: var(--svg-color--23addd, #23addd);
}

.ft5dv1b6b {
  fill: none;
}

.x66lllhpd {
  fill-rule: evenodd;
  d: path("m11.928 14.055l.972.846c1.048.99 2.124 1.656 3.708 1.597a4.454 4.454 0 0 0 4.392-4.5c0-2.475-1.967-4.5-4.392-4.5c-1.062 0-2.084.257-2.849.945l-1.102 1.08l.585.59l1.008-.99c.62-.558 1.395-.725 2.25-.725c1.957 0 3.6 1.607 3.6 3.6s-1.593 3.6-3.55 3.6c-1.279.045-2.116-.504-2.966-1.305l-.968-.994l.653-.675v.004l1.89-1.899a1.845 1.845 0 0 1 2.578 2.628c-.715.725-1.908.495-2.497-.153l-.742-.693l-.662.693l.733.747c.981.945 2.835 1.058 3.87 0a2.745 2.745 0 0 0-.036-3.825a2.817 2.817 0 0 0-3.964-.013l-4.072 4.126c-.846.801-1.638 1.409-2.916 1.36A3.556 3.556 0 0 1 3.9 12.02a3.59 3.59 0 0 1 3.55-3.623c.855 0 1.678.216 2.295.774l1.637 1.593l-2.065 2.043l-.553.567a1.82 1.82 0 0 1-2.192.315l1.539-1.566l1.575-1.638l-.351-.36a2.565 2.565 0 0 0-3.753 0a2.74 2.74 0 0 0 0 3.825a2.62 2.62 0 0 0 3.753 0l3.204-3.2l-.562-.566l-1.737-1.692c-.765-.693-1.787-.945-2.849-.945A4.43 4.43 0 0 0 3 12.016c0 2.47 1.967 4.473 4.392 4.473c1.584.059 2.623-.603 3.672-1.593l.864-.846zM6.2 10.729c-.626.64-.63 1.476-.167 2.205l2.309-2.385c-.779-.517-1.517-.459-2.142.18");
}

.ymshwgb8g {
  stop-color: var(--svg-color--28a6e1, #28a6e1);
}

</style>
