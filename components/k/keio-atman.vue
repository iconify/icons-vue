<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="b7dcu0b4s"/><path class="wx-pguerm"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:keio-atman" /></template>
<style>
.b7dcu0b4s {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M6.678 9.722c-.87-2.335 2.104-3.61 4.045-3.703a120 120 0 0 0 4.725-.333c3.125-.29 21.013-.224 21.907.081c1.153.309 3.068 4.3.828 4.3c-1.854-.465-20.434-.055-23.302.514c-1.091.217-2.686.539-3.543.716c-2.931.714-3.77.548-4.66-1.575m25.723 26.304c-3.151-4.384-6.182-9.083-9.829-12.368c-1.526 1.526-6.11 9.217-6.853 11.819c4.025-.16 11.412-.65 16.682.55z");
}

.wx-pguerm {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("m13.335 40.216l-.841.921c-1.116.847-3.076 1.173-4.559.606c-1.607-.928-.231-3.251.937-5.274c1.962-3.4 7.498-13.609 10.424-16.725c-1.953-2.098-3.957-3.512-5.837-5.416c-.829-.829 1.147-3.54 2.307-3.826c1.093-.206 1.784.28 4.557 3.205c1.947 2.053 2.033 2.108 2.561 1.63c.863-.863 1.298.203.927 1.59c1.295-1.458 1.736-1.132 2.41-.5c.4.376.162 1.888-.234 2.987c2.562 2.562 10.925 13.063 15.317 20.671c.394 1.061.065 2.019-.46 2.258c-.603.273-1.21.154-1.725-.08c-8.124-2.795-18.916-2.255-25.784-2.047");
}

</style>
