<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":48,"height":48};
const content = `<path class="jglr2ib7i"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="arcticons:vk-x" /></template>
<style>
.jglr2ib7i {
  fill: none;
  stroke: currentColor;
  stroke-linecap: round;
  stroke-linejoin: round;
  d: path("M5.5 15.005h21.629M5.5 22.713h21.629M5.5 30.421h15.961m12.333-12.333l7.777-1.377c.537-.096.929-.563.929-1.108v-4.134c0-.707-.643-1.238-1.337-1.105L34.707 11.6c-.53.102-.913.566-.913 1.105V30.42c0 1.95.295 6.02-3.276 6.904c-4.125 1.02-5.68-.409-5.781-2.993c-.102-2.585 2.04-3.605 4.455-4.149l2.14-.483a3.156 3.156 0 0 0 2.462-3.087v-.68");
}

</style>
