<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="p68yjjb4v"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:face-shake-sharp" /></template>
<style>
.p68yjjb4v {
  fill: currentColor;
  d: path("M7.475 22.225Q5.3 21.45 3.5 19.975V22H2v-5h5v1.5H4.1q1.65 1.45 3.675 2.225T12 21.5t4.225-.775T19.9 18.5H17V17h5v5h-1.5v-2.025q-1.8 1.475-3.975 2.25T12 23t-4.525-.775M12 17q-3.35 0-5.675-2.325T4 9t2.325-5.675T12 1t5.675 2.325T20 9t-2.325 5.675T12 17m0-2q2.5 0 4.25-1.75T18 9q0-.275-.025-.525T17.9 7.95q-1.85.2-3.575-.413T11.3 5.6q-.95 1.3-2.337 2.15T6 8.875Q5.95 11.4 7.7 13.2T12 15m-2.5-4q-.425 0-.712-.288T8.5 10t.288-.712T9.5 9t.713.288t.287.712t-.288.713T9.5 11m5 0q-.425 0-.712-.288T13.5 10t.288-.712T14.5 9t.713.288t.287.712t-.288.713T14.5 11");
}

</style>
