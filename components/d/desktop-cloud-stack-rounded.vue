<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="ch68k-dsa"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols-light:desktop-cloud-stack-rounded" /></template>
<style>
.ch68k-dsa {
  fill: currentColor;
  d: path("M7.116 15.48h6.5q.84 0 1.42-.59t.58-1.432t-.597-1.422t-1.453-.58h-.27v-.115q-.046-1.237-.973-2.049t-2.207-.811q-.881 0-1.611.473t-1.097 1.263l-.135.27l-.288.019q-1.008.05-1.689.758q-.68.709-.68 1.711q0 1.044.729 1.775t1.77.73M8.232 22q-.213 0-.356-.144t-.144-.357t.144-.356T8.23 21h.923v-2.808H3.616q-.691 0-1.153-.462T2 16.577V7.385q0-.69.463-1.153t1.152-.463h13q.69 0 1.153.463t.463 1.153v9.192q0 .69-.463 1.153t-1.152.462h-5.539V21H12q.213 0 .356.144t.144.357t-.144.356T12 22zM21 12.942V3.616q0-.27-.173-.443T20.385 3H6.308q-.213 0-.356-.144t-.144-.357t.144-.356T6.308 2h14.077q.69 0 1.152.463T22 3.616v9.326q0 .213-.144.357t-.357.143t-.356-.143t-.143-.357");
}

</style>
