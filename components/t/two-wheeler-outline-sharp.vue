<script setup>
import { Icon } from '@iconify/css-vue';

const props = defineProps(["width","height"]);

const viewBox = {"width":24,"height":24};
const content = `<path class="s2igrtbuk"/>`;
</script>
<template><Icon :width="width" :height="height" :viewBox="viewBox" :content="content" fallback="material-symbols:two-wheeler-outline-sharp" /></template>
<style>
.s2igrtbuk {
  fill: currentColor;
  d: path("M4 19q-1.65 0-2.825-1.175T0 15q0-1.425.913-2.525T3.25 11.1l-.7-.6H0V9h4.5L7 10.5L11 9h3.15L12.6 7H10V5h3.55l2.1 2.7L19 6v3h-2.3l1.75 2.3q.375-.15.763-.225T20 11q1.65 0 2.825 1.175T24 15t-1.175 2.825T20 19t-2.825-1.175T16 15q0-.675.238-1.312T16.9 12.5l-.5-.6L13 17h-3l-2-1.75q-.125 1.575-1.275 2.663T4 19m0-2q.825 0 1.413-.587T6 15t-.587-1.412T4 13t-1.412.588T2 15t.588 1.413T4 17m7.35-6l-3.6 1.35zh3.25zM20 17q.825 0 1.413-.587T22 15t-.587-1.412T20 13t-1.412.588T18 15t.588 1.413T20 17m-8.05-2l2.65-4h-3.25l-3.6 1.35l3 2.65z");
}

</style>
